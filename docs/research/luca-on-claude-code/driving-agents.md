...
- **Claude Code workflows are not a launch path for the engine.** A model turn has to start one, and the script can't run gates, git, or any I/O.
...
- **Agent SDK.** The `query()` generator yields every message: assistant and user messages, tool results, `system/init`, `api_retry`, task progress for subagents, and the final `result` ([message types](https://code.claude.com/docs/en/agent-sdk/typescript.md)). `includePartialMessages` adds token-level stream events ([streaming output](https://code.claude.com/docs/en/agent-sdk/streaming-output.md)).
  - **Tokens.** Each assistant message carries `message.usage`; dedupe by `message.id`. The result has `usage`, per-model `modelUsage`, and `total_cost_usd`, which is an estimate. In streaming-input mode, each turn emits its own result, and `modelUsage` is a running total ([cost tracking](https://code.claude.com/docs/en/agent-sdk/cost-tracking.md)).
...
- **Paseo.**
  - Cancel: the public agent handle has no cancel method ([SDK reference](https://paseo.sh/docs/sdk/reference)). The internal client has `cancelAgent()` (`packages/client/src/daemon-client.ts:3418`), the CLI has `paseo stop <id>`, and MCP has `cancel_agent`. `archive()` also interrupts.
...
- **No direct API.** An agent's attention reason is one of `finished`, `error`, or `permission` (`packages/protocol/src/messages.ts:766-770`). The daemon sets it only when an agent goes from running to idle, enters an error, or gets a permission request (`agent-manager.ts:4770-4795`, and `:4530`). The protocol has requests to clear attention, but none to raise it.
...
- **Route 2: a Paseo terminal.** A process started in a Paseo terminal gets `PASEO_TERMINAL_ID`, `PASEO_ACTIVITY_TOKEN`, and `PASEO_TERMINAL_ACTIVITY_URL` (`terminal/terminal-manager.ts:333-341`). `paseo hooks claude Notification`, fed an `idle_prompt` payload, posts "needs input" to that URL (`packages/cli/src/commands/hooks.ts`, `terminal/agent-hooks/claude/claude.ts:30-31`). The daemon then raises "Terminal needs input", with the terminal's name as the body (`websocket-server.ts:188-190`, `:2624-2660`). The hook is meant for agent CLIs, so this is a borrowed use.
...
The code is old Luca's `packages/luca-code` (tag `old-luca-final`); the built file is `packages/luca-code/dist/cc-openai-bridge.js` in the main checkout (built 2026-07-23, not tracked).

- **How it launches.** `cc-openai-bridge claude [args…]` starts a loopback gateway, then spawns `claude` with the args passed through (`dist/cc-openai-bridge.js:16632-16651`; README at the tag). The child gets `ANTHROPIC_BASE_URL` (the gateway), `ANTHROPIC_AUTH_TOKEN` and `ANTHROPIC_API_KEY` (a per-launch gateway token), model pins such as `ANTHROPIC_MODEL`, and `CLAUDE_CONFIG_DIR` set to the profile (`old-luca-final:packages/luca-code/src/launcher/claude.ts:269-309`). It also adds `--managed-settings` and rejects that flag from callers (same file). Its settings come from `CCOB_*` env vars (`dist/cc-openai-bridge.js:13587-13614`).
- **No standalone gateway.** The commands are `login`, `claude`, `status`, `logout`, `help`, and `version` (help text, `dist/cc-openai-bridge.js:16632-16651`). The gateway lives only as long as the `claude` it launched. So setting `ANTHROPIC_BASE_URL` alone won't work; each launch must go through the bridge.
- **`claude -p`: yes, in principle.** Run `cc-openai-bridge claude -p … --output-format stream-json`; the bridge passes the flags through. It rewrites `--model` values it doesn't know to its selected model (`launcher/claude.ts` at the tag).
- **Agent SDK: probably.** Point `pathToClaudeCodeExecutable` at a wrapper that runs `cc-openai-bridge claude "$@"`, or use `spawnClaudeCodeProcess` ([Options](https://code.claude.com/docs/en/agent-sdk/typescript.md)). The bridge spawns `claude` with inherited stdio, so the SDK's pipes should pass through. Untested.
...
- **Who can steer.** Only the internal client's `sendAgentMessage(…, { activeTurnBehavior })` sets it (`daemon-client.ts:360-365`, `:3342-3359`). The public `PaseoAgentSendOptions` type omits it (`packages/client/src/index.ts:279-283`). The MCP `send_agent_prompt` tool and `paseo send` never set it, so they always interrupt (`paseo-tools.ts:1868-1900`; `paseo send --help`). Paseo's own schedules use `"steer"` (`schedule/service.ts:860`).
- **Docs.** The `v0.9.1` docs no longer say a prompt doesn't interrupt; #330 quoted that line from an older page.
...
4. **The Agent SDK under Bun.** Run one `query()` from a Bun process, streaming input, with `interrupt()` and `streamInput()`.
