...
- **Only the OS sandbox holds for Bash, and it covers Bash only.** It stops writes outside the worktree, all network (so no `git push`, no `gh`, no MuninnDB over loopback), and edits to settings and hook files. It does not cover the Edit tool, MCP tools, or hooks.
...
- **Writes.** By default, sandboxed commands can write the working directory, the session temp dir, and added directories. Reads are allowed everywhere, credential files included ([filesystem isolation](https://code.claude.com/docs/en/sandboxing#filesystem-isolation)).
...
- **Network.** All traffic goes through a proxy. "Claude Code pre-allows no domains by default." With `strictAllowlist`, or in `dontAsk` mode, hosts off the list are denied instead of prompting ([network isolation](https://code.claude.com/docs/en/sandboxing#network-isolation), [strictAllowlist](https://code.claude.com/docs/en/settings-reference#sandbox-network-strictallowlist)). On macOS, "the Seatbelt profile allows communication only to a specific localhost port", and the proxy refuses loopback addresses. But `allowLocalBinding` "lets the sandboxed process connect to loopback ports without going through the proxy at all" ([sandbox-runtime README](https://github.com/anthropic-experimental/sandbox-runtime/blob/ddbeb74711c4097014ef3056791efa83f553116c/README.md)).
...
- **Custom providers** in `~/.paseo/config.json` can set `command`, `env`, `disallowedTools`, and a `paseoTools` policy per provider ID (`packages/protocol/src/provider-config.ts:51-64`; [custom providers](https://paseo.sh/docs/custom-providers)). A `command` array becomes a "replace" launch whose extra elements go before the SDK's own arguments (`provider-registry.ts:261-276`, `providers/claude/query.ts:32-55`). So a provider such as `"command": ["claude", "--strict-mcp-config"]` would add that flag. This is a hack; the doc warns flags passed twice may resolve by position (untested).
...
| Plugins | enabled plugins' `.mcp.json` or `plugin.json` | no (plugins come from settings) | strict config, or dropping `user` |
...
- **Binary-independent path scanning** for the handoff mailbox: any token mentioning the path counts, whatever program carries it (`classify-bash-command.ts:764-798`). It caught `bun -e` and `tee` forgeries a per-program extractor missed.
...
- `hooks.ConfigChange`: block every change.
...
- **#340** (memory): answered in §3. Strict MCP plus a loopback-closed sandbox keeps MuninnDB out.
