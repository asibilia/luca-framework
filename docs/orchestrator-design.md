...
> **Status:** DESIGN PASS — 2026-05-22. Locks four decisions (D1–D4) from the
> user; sketches schemas, contracts, telemetry, and a migration path. No
...
This mirrors the prototype's halt-then-resume cadence but makes the
verifier verdict an explicit state branch.
...
| `luca-bridge orchestrator transition` | `--event=<EVENT>` `[--data=<json>]` | Apply one of the legal transitions in §4.3. Events: `WORKER_RETURNED`, `VERIFIER_PASS`, `VERIFIER_BLOCKER`, `VERIFIER_FYI`, `ADVANCE`, `HALT`, `CLEAR_HALT`, `COMPLETE`. Each carries the relevant metadata in `data` (e.g. commit sha, halt reason). |
...
| `luca-bridge orchestrator clear` | — | Reset `orchestration` to `undefined`. Operator-only — for aborting cleanly. |

The set is intentionally small (6 commands). Anything beyond this should
go through a higher-level subagent/skill — the bridge is not a general
programming surface, it's a typed gate.
...
1. **Environment + lock pre-flight.**
   ...
   - Snapshot `PRE_AGENT_HEAD = git rev-parse HEAD`.
   - Touch the orchestrator lock file (`.luca/orchestration.lock`,
     PID-based; refuse if a different live PID holds it).

2. **Read cursor via bridge.**
   - `luca-bridge orchestrator read-cursor` → cursor JSON.
   - If `status === 'IDLE'`, call `init` with the resolved
     `manifest_id` and read again.

3. **Pre-flight halt-checks** (in order, halt on first hit):
...
4. **Pick next increment.**
   - From the manifest, find the row matching `(phase, step)` or — for
     a resume after partial progress — `(phase, step, substep)`.
   - Bridge: `orchestrator set-increment --id=… --phase=… --step=…
     --substep=…`.
   - Bridge: `orchestrator transition --event=WORKER_RETURNED` is
     deferred until §5.1.6 — at this point we just mark status
     `RUNNING` (or leave RUNNING if a partial resume).

5. **Spawn worker** (foreground; the prototype's "Foreground sub-agent
   (recommended)" decision holds).
   - `worker.kind === 'phase-run'` → `Skill(skill: "phase-run", args:
     "<title> <flags>")`. The orchestrator passes `--orchestrated`
     so `/phase-run` knows not to mint its own roadmap/branch.
   - `worker.kind === 'subagent'` → `Task(agent: <subagent_id>,
     prompt: <prompt_template w/ increment fields substituted>)`.
   - `worker.kind === 'general-purpose'` → `Task(subagent_type:
     'general-purpose', prompt: <substituted>)`.
   - Emit telemetry: `luca telemetry emit --kind=orchestrator.iteration.start
     --run-id=<run_id> --meta='{"increment_id": "…", "worker":
     "phase-run"}'`.

6. **Wait for worker.** Foreground call — no async dance. On return:
   - `POST_AGENT_HEAD = git rev-parse HEAD`.
   - `COMMITS = git log --pretty=format:"%H %s" PRE..POST`.
   - `DIFFSTAT = git diff --stat PRE..POST`.
   - `DELETIONS = git log --diff-filter=D --name-only PRE..POST | sort -u`.
   - Capture last ~500 chars of the worker's final message (the Task
     tool returns this as the agent's response body).
   - Bridge: `orchestrator transition --event=WORKER_RETURNED
     --data='{"post_head": "<sha>"}'`. Status becomes `VERIFYING`.

7. **Spawn `orchestrator-verifier`** (new — see §6).
   - Input record assembled from §5.1.6 capture: increment scope,
     PRE..POST commits, diffstat, deletions, worker tail, cursor PRE
     → POST diff.
   - Output verdict drives the next state branch:
     - **PASS** → `transition --event=VERIFIER_PASS` (status →
       `ADVANCED`).
     - **FYI** → `transition --event=VERIFIER_FYI`; orchestrator
       calls `append-fyi` with the rationale; status → `ADVANCED`.
     - **BLOCKER** → `transition --event=VERIFIER_BLOCKER --data=
       '{"halt_reason": "<rationale>"}'`; status → `HALTED`.

8. **Post-flight structural halt-checks** (run when status was about
   to become `ADVANCED`; these mirror the prototype's §6 checks):
   ...
   If any fire, transition to `HALTED` with the matching reason.

9. **Emit telemetry — both surfaces.**
   - JSONL: `luca telemetry emit --kind=orchestrator.iteration.end
     --run-id=<run_id> --duration-ms=<ms>
     --meta='{"increment_id": "…", "verdict": "PASS",
              "files_touched": <n>, "commits": <n>,
              "halt_reason": null}'`.
     For HALTED: `kind=orchestrator.halt`, with the halt reason.
     For each FYI: `kind=orchestrator.verifier.verdict` with
     `meta.verdict = "FYI"`.
   - MuninnDB engram: `mcp__muninn__muninn_remember(vault:
     <repo-vault>, concept: "metric:luca-iteration-<run_id>-<n>",
     content: <iteration summary; see §7.2 for the schema>,
     type: "metric", summary: "<one-line>", entities: [
       <increment_id>, <worker>, <verdict>])`.

10. **Advance cursor via bridge.**
    - On PASS or FYI: `transition --event=ADVANCE --data='{
        "last_completed_id": "<id>",
        "last_completed_commit": "<sha>",
        "next_increment_id": "<next>",
        "next_phase": "<p>", "next_step": <n>}'`. The bridge enforces
      that `next` is a legal step in the manifest.
    - On HALT: the transition in §5.1.7 or §5.1.8 already moved the
      cursor to `HALTED`; also emit a `PushNotification` (status:
      `proactive`) to surface the halt to the user.
    - On manifest-exhausted: `transition --event=COMPLETE`; status
      becomes `COMPLETE`; emit `kind=orchestrator.manifest.complete`
      telemetry; release the orchestrator lock.

11. **ScheduleWakeup if running under `/loop`.**
    - The harness re-fires; the wakeup is a no-op outside `/loop`.
    - Default interval: 1200 seconds (20 minutes), tunable per
      preset via `lucaConfig.orchestrator.wakeup_seconds`.

12. **Return** to the caller with a one-line summary:
    `"✓ <increment_id> [PASS|FYI] — cursor at <phase>-<step>. Next:
      <next_id>."`.
...
| §5 sub-agent spawn (general-purpose only) | Worker discriminated union — phase-run, subagent, or general-purpose |
...
| (no inter-iteration verifier) | **New** §5.1.7 — `orchestrator-verifier` between worker and advance |
| (telemetry not emitted from driver) | **New** §5.1.9 — JSONL + MuninnDB summary engram every iteration |

### 5.3 What stays the same

- **Foreground worker spawn.** Background workers caused the prototype's
  early flake (race between cursor read and worker write). Stay
  foreground until proven otherwise.
...
export const OrchestratorVerifierInputSchema = z.object({
  ...
  worker: z.object({
    ...
    final_message_tail: z.string(),  // last ~500 chars
  }),
...
Nine halt conditions. The first eight come from the prototype; #9 is
new.

| # | Halt | Detection | PushNotification template | Resume procedure |
| --- | --- | --- | --- | --- |
| 1 | Pre-flight environment fault | Wrong branch / dirty modifications / missing bun (§5.1.3 #1, #6) | `"luca orchestrator halted: environment fault (<detail>). See state.json orchestration.halt_reason."` | Operator fixes branch/cleans tree; re-fires `/luca`. |
| 2 | Cursor halt_reason non-empty (prior halt) | §5.1.3 #2 | `"luca orchestrator resuming halted state: <reason>. Run `luca-bridge orchestrator transition --event=CLEAR_HALT` to retry."` | Operator inspects, calls `CLEAR_HALT`, re-fires. |
| 3 | Manifest exhausted / terminal boundary | §5.1.3 #3, #5 | `"luca orchestrator complete: manifest <id> finished after <n> iterations (<m> halted)."` | None — clean exit. |
| 4 | Worker silent failure | §5.1.8 #1 (no commits, no halt marker) | `"luca orchestrator halted at <increment>: worker returned with no diff and no halt marker."` | Operator investigates worker logs; re-fires after fix. |
| 5 | Missing cursor advance | §5.1.8 #2 | `"luca orchestrator halted at <increment>: worker did not advance cursor."` | Operator inspects state.json + memory; re-fires. |
| 6 | Unauthorized deletion | §5.1.8 #3 | `"luca orchestrator halted at <increment>: worker deleted <paths> outside authorized_deletions."` | Operator triages — typically revert and re-fire with a refined whitelist. |
| 7 | Repeated tsc fail | §5.1.8 #4 (hash repeats two iterations) | `"luca orchestrator halted at <increment>: tsc failure hash <h> repeated twice."` | Operator fixes tsc; re-fires. |
| 8 | Cursor points at unknown increment | §5.1.3 #4 | `"luca orchestrator halted: cursor points at <phase>-<step> which is not a defined increment."` | Operator reconciles cursor (typically via `set-increment`); re-fires. |
| 9 | **Verifier BLOCKER** | §5.1.7 | `"luca orchestrator halted at <increment>: verifier verdict BLOCKER. Rationale: <one-line>."` | Operator reads verifier audit; either accepts and adjusts the manifest, or fixes the worker output and re-fires. |
...
3. Emits `kind=orchestrator.halt` telemetry.
...
- The cursor format — though the storage moves from a MuninnDB body
  block into the typed state.json slice (§4.2).
- The Foreground-sub-agent decision.
...
12. **Lock contention.** The orchestrator takes a lock at §5.1.1; the
    ...
    lock to the worker? Defer until §5.1.5 worker-kind = phase-run
    actually exists.
...
These are the four decisions the design pass treats as given. They
are NOT re-debated in this doc.
