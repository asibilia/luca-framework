...
| 7i       | Checks Fix Loop       | No      | harness-{NN}, fix-{NN}                | Up to CHECKS_FIX_ITERATIONS attempts      |
...
### Step 7h-i: Execution + Checks Fix Loop

The executor runs wave tasks. After execution, checks run mechanical checks (typecheck, build). If checks fail, a fix agent attempts repairs, and checks re-run. This iterates up to `checksFixIterations`.
