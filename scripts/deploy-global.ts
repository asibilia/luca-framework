...
function deployDir(
  ...
  if (mode === "symlink") {
    ...
    copyDirRecursive(source, target, sourceType);
...
function copyDirRecursive(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (sourceType) {
        deployedFiles.push({
          relativePath: relative(GLOBAL_DIR, tgtPath),
...
async function deployHooks(projectRoot: string): Promise<number> {
  ...
  for (const script of scripts) {
    ...
    rewriteWrapperPaths(targetPath, projectRoot);
...
async function deployStatusline(projectRoot: string): Promise<void> {
  ...
  // Rewrite relative $(dirname "$0")/../ paths to absolute monorepo paths
  rewriteWrapperPaths(target, projectRoot);
