...
/**
 * Validate Cursor adapter output against Cursor IDE constraints.
 *
 * Checks:
 ...
 *
 * @param emitResult - The EmitResult from the Cursor adapter's emit() call
 * @returns A CompatibilityReport for the Cursor adapter
 *
 * @example
 * ```typescript
 * const adapter = createCursorAdapter();
 * const emitResult = await adapter.emit(outputDir);
 * const report = await validateCursorOutput(emitResult);
 * console.log(report.fully_compatible); // true if no issues found
 * ```
 */
export async function validateCursorOutput(
  ...
  // --- Rules ---
  {
    ...
    for (const filePath of files.rules) {
      ...
      // Check for valid .mdc YAML frontmatter
      if (filePath.endsWith(".mdc")) {
        const fm = extractFrontmatter(content);
        if (!fm) {
          warnings.push(`Rule file missing YAML frontmatter: ${filePath}`);
          degradedCount++;
        } else if (!fm.description) {
          warnings.push(
            `Rule file missing 'description' in frontmatter: ${filePath}`,
          );
          degradedCount++;
        }
      }
    ...
    features.push({
      ...
      notes:
        degradedCount > 0
          ? `${degradedCount} rule(s) with frontmatter issues`
          : "",
  ...
  // --- Skills ---
  {
    ...
    for (const filePath of files.skills) {
      ...
      // Check that skill files are SKILL.md in subdirectories
      if (!filePath.endsWith("SKILL.md") && !filePath.endsWith("skill.md")) {
        warnings.push(
          `Skill file not in expected SKILL.md format: ${filePath}`,
        );
        degradedCount++;
      }
    }
    ...
    features.push({
      ...
      notes:
        degradedCount > 0 ? `${degradedCount} skill(s) with format issues` : "",
  ...
  // --- Hooks ---
  {
    const warnings: string[] = [];
    let degradedCount = 0;
    // Known Cursor event names (from CURSOR_EVENT_MAP values)
    ...
    for (const filePath of files.hooks) {
      ...
      // Check that hook config files are valid JSON
      if (filePath.endsWith(".json")) {
        try {
          const parsed = JSON.parse(content);
          // Check for known event names if the JSON has event fields
          if (parsed && typeof parsed === "object") {
            const eventKeys = Object.keys(parsed);
            for (const eventKey of eventKeys) {
              if (eventKey !== "_warning" && !knownEvents.has(eventKey)) {
                warnings.push(
                  `Hook file contains unknown event '${eventKey}': ${filePath}`,
                );
                degradedCount++;
              }
            }
    ...
    features.push({
      ...
      notes:
        degradedCount > 0
          ? `${degradedCount} hook(s) with validation issues`
          : "",
  ...
  // --- Agents ---
  // Cursor has no dedicated agent format (agents: false in supportedFeatures)
  {
    features.push({
      feature: "agents",
      status: files.agents.length > 0 ? "partially_mapped" : "unsupported",
      notes:
        "Cursor has no dedicated agent format; agents compiled as markdown rules",
      item_count: files.agents.length,
      degraded_count: 0,
      warnings:
        files.agents.length > 0
          ? [
              `${files.agents.length} agent file(s) emitted but Cursor has no agent format`,
            ]
          : [],
    });
  }
  ...
  return buildReport("cursor", "Cursor IDE", "1.0.0", "Cursor", features);
}
 ...
 * @example
 * ```typescript
 * const cursorReport = await validateCursorOutput(cursorEmit);
 * const windsurfReport = await validateWindsurfOutput(windsurfEmit);
 * const vscodeReport = await validateVscodeOutput(vscodeEmit);
 *
 * const aggregated = aggregateReports([cursorReport, windsurfReport, vscodeReport]);
