...
// level. This is complementary to the per-tool-name throttle below -- guardDedup
// prevents the same hook invocation from running twice within 5s, while the
// throttle prevents repeated recordings of the same error pattern within 5 min.
guardDedup("post-tool-use-failure");
