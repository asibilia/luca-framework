...
export default function lucaWorkTracking(pi: any) {
    ...
    parameters: {
      ...
      properties: {
        ...
        title: {
          type: "string",
          description:
            "Title for a new inline todo (required when todo='new')",
        },
        area: {
          type: "string",
          description:
    ...
    async execute(
      ...
      return createJsonResponse({
        ...
        message: state.tracked
          ? `✓ Work tracked: #${state.issueNumber} on branch ${state.branch}`
  ...
  pi.registerTool({
    ...
    async execute() {
      ...
      return createJsonResponse({
        ...
        title: state.todoTitle,
        issue: state.issueNumber
          ? { number: state.issueNumber, url: `https://github.com/${getRepoSlug()}/issues/${state.issueNumber}` }
          : null,
        branch: state.branch,
        mode: state.mode,
        warnings_issued: warningCount,
        checklist: {
          todo: state.todo !== null ? "✓" : "✗ — use luca_track_work",
          issue:
            state.issueNumber !== null
              ? "✓"
              : "✗ — use luca_track_work or luca_link_issue",
          branch:
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId: string, params: { area?: string }) {
      if (!existsSync(todosDir)) {
        return createJsonResponse({ todos: [], message: "No pending todos directory found." });
      }
  ...
  pi.on("tool_call", async (event: any, ctx: any) => {
    ...
    const warning =
      `⚠️ Untracked change detected — missing: ${missing.join(", ")}. ` +
    ...
    if (state.mode === "warn" && warningCount < MAX_WARNINGS) {
      warningCount++;
      if (ctx?.ui?.setStatus) {
        ctx.ui.setStatus("luca-tracking", `⚠️ UNTRACKED${warningCount >= MAX_WARNINGS ? " (muted)" : ""}`);
      }
    }
  });
  ...
  pi.on("session_start", async (_event: any, ctx: any) => {
    ...
    if (ctx?.ui?.setStatus) {
      if (state.tracked) {
        ...
        ctx.ui.setStatus("luca-tracking", `📋 #${state.issueNumber} (no todo linked)`);
      }
    }
  });
