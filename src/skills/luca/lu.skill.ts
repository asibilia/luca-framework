/**
 * lu Skill — Thin orchestrator for all Luca workflows.
 *
 * Decomposes into 4 sub-skills: lu-route, lu-configure, lu-backlog, lu-phase-loop.
 * This orchestrator contains ONLY Skill() calls, context reads, state writes,
 * and arg parsing. All business logic lives in sub-skills.
 *
 * **State machine:** idle -> routed -> configured -> scanned -> executing -> complete
 * **Context file:** /tmp/lu-context.json
 *
 * **CRITICAL (Pitfall 1):** Write `current_state` to context file after EVERY
 * state transition to keep the pre-step enforcement hook in sync.
 *
 ...
 * @see .planning/phases/224-anti-skip-rollout/04-PLAN.md Task 9
 */
...
const luSkillConfig: SkillConfig = {
  ...
  sections: [
    {
      title: "main",
      content: `**Branding:** Read \`.planning/config.json\` branding section at session start. Use \`/{commandPrefix}\` instead of \`/lu\` and \`{frameworkName}\` instead of \`Luca\` in ALL user-facing output. If config is missing, fall back to defaults (\`/lu\`, \`Luca\`).
