/**
 * lu Skill - Unified entry point and autonomous orchestrator for all Luca workflows.
 *
 * Combines routing (for quick/debug/PR tasks) with autonomous orchestration
 * (for phase/milestone work: backlog scan, roadmap revision, multi-phase execution).
 */
...
const luSkillConfig: SkillConfig = {
  ...
  sections: [
    {
      title: "main",
      content: `**Branding:** Read \`.planning/config.json\` branding section at session start. Use \`/{commandPrefix}\` instead of \`/lu\` and \`{frameworkName}\` instead of \`Luca\` in ALL user-facing output (logs, summaries, help text, error messages). If config is missing or branding is absent, fall back to defaults (\`/lu\`, \`Luca\`).
...
`,
    ...
    },
    {
      ...
      content: `This skill uses TWO delegation mechanisms and serves as both a **router** (for quick/debug/PR tasks) and a **meta-orchestrator** (for phase/milestone work).
...
\`\`\``,
    ...
    {
      title: "backlog_scan",
      content: `## Step 1: Backlog Scan & Unplanned Detection
...
If UNPLANNED_COUNT == 0: Skip to Step 3.`,
    ...
    {
      title: "roadmap_revision",
      content: `## Step 2: Roadmap Revision
