/**
 * milestone-prune Sub-Skill — Detect and prune stale MuninnDB engrams.
 *
 * Extracts Step 0.5 (stale memory detection and pruning) from the monolithic
 * milestone-complete skill.
 *
 * **Responsibility:** Recall engrams and metrics, identify stale engrams using
 * the 5+/3+ threshold, present to user for review, prune approved engrams,
 * consolidate near-duplicates, and write results to the shared context file.
 *
 * **Input:** Milestone version (from orchestrator context)
 * **Output:** Populated `milestone_prune` section in `/tmp/milestone-complete-context.json`
 *
 * @see .planning/phases/224-anti-skip-rollout/01-PLAN.md Task 4
 */
import { createSkill } from "~/skills/__helpers/create-skill";

import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const milestonePruneConfig: SkillConfig = {
  frontmatter: {
    name: "milestone-prune",
    description:
...
export const milestonePruneSkill = createSkill(milestonePruneConfig);
