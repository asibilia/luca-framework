/**
 * milestone-shadow-gate Sub-Skill — Pre-archive shadow debt scan gate.
 *
 * Extracts Step 0.7 (shadow debt scan) from the monolithic milestone-complete
 * skill.
 *
 * **Responsibility:** Check if shadow scanning is enabled in config, spawn
 * lu-shadow-scanner if enabled, present critical findings to user, and write
 * results to the shared context file.
 *
 * **Input:** Milestone version (from orchestrator context)
 * **Output:** Populated `milestone_shadow_gate` section in `/tmp/milestone-complete-context.json`
 *
 * This is an OPTIONAL step. The orchestrator sends SKIP_SCAN if shadow scanning
 * is disabled in config.
 *
 * @see .planning/phases/224-anti-skip-rollout/01-PLAN.md Task 5
 */
import { createSkill } from "~/skills/__helpers/create-skill";

import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const milestoneShadowGateConfig: SkillConfig = {
  frontmatter: {
    name: "milestone-shadow-gate",
    description:
...
### Step 2: Spawn lu-shadow-scanner
...
- **lu-shadow-scanner spawn failure:** Log warning, set \`shadow_scan_ran: false\`, \`gate_result: "error"\`, write to context, and return.
...
- When called, it MUST spawn lu-shadow-scanner (do not skip the scan)
...
export const milestoneShadowGateSkill = createSkill(milestoneShadowGateConfig);
