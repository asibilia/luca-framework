/**
 * phase-execute-verify Sub-Skill — Run harness and verification fix loops.
 *
 * Extracts Steps 5-7 from the monolithic phase-execute skill (verification loops).
 *
 * **Responsibility:** Run Loop A (harness mechanical fix: run harness, if failures
 * spawn executor to fix, re-run up to max iterations), then run Loop B (verify
 * semantic fix: spawn lu-verifier, if issues spawn executor to fix, re-verify up
 * to max iterations), and aggregate verification results.
 *
 * **Input:** Phase number, complexity level (from orchestrator args/context)
 * **Output:** Populated `phase_execute_verify` section in `/tmp/phase-execute-context.json`
 *
 * @see .planning/phases/224-anti-skip-rollout/03-PLAN.md Task 4
 */
import { createSkill } from "~/skills/__helpers/create-skill";

import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const phaseExecuteVerifyConfig: SkillConfig = {
  frontmatter: {
    name: "phase-execute-verify",
    description:
...
3. If harness fails and iterations remaining:
   ...
   - Spawn lu-executor to fix the identified issues:
     \`\`\`
     Task(
       prompt: """
     <fix_context>
...
Spawn lu-verifier for semantic verification of the phase goal:
...
3. If verification finds issues and iterations remaining:
   - Spawn lu-planner + lu-plan-checker to create fix plans
   - Spawn lu-executor to execute fix plans
   - Re-run lu-verifier
   - Repeat until pass or iterations exhausted
...
- **lu-verifier spawn failure:** Log error and treat as verify_passed = false. Do not ABORT.
- **lu-executor fix spawn failure:** Log error, decrement remaining iterations, continue loop.
...
- Spawn lu-executor for fixes — do NOT fix code inline
- Spawn lu-verifier for semantic verification — do NOT verify inline
...
export const phaseExecuteVerifySkill = createSkill(phaseExecuteVerifyConfig);
