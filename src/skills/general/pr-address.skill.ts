/**
 * pr-address Skill — Thin orchestrator for the PR comment address workflow.
 *
 * Delegates all work to 6 sub-skills via Skill() calls:
 *   pr-fetch, pr-validate, pr-debate, pr-fix, pr-learn, pr-respond
 *
 * Contains ONLY:
 *   - Skill() calls to sub-skills
 *   - Context file reads via readPrContext()
 *   - State machine transitions (prAddressStateMachine events)
 *   - Arg parsing and flag handling
 *   - Vault routing and model resolution (passthrough config)
 *
 * Contains NO inline logic:
 *   - No `gh api` calls (moved to pr-fetch, pr-respond)
 *   - No Task() spawns (moved to pr-validate, pr-debate, pr-fix, pr-learn)
 *   - No YAML parsing or template interpolation
 *   - No comment categorization logic
 *
 * **PREMORTEM Constraint #3:** Zero inline logic — only Skill() calls +
 * context reads + state transitions.
 *
 * @see .planning/phases/223-anti-skip-pilot/01-CONTEXT.md
 * @see .planning/phases/223-anti-skip-pilot/01-PREMORTEM.md
 */
