/**
 * pr-fix Sub-Skill — Plan fixes, execute them, and verify they address concerns.
 *
 * Extracts Steps 5-6-7 from the monolithic pr-address skill.
 *
 * **Responsibility:** Read valid concerns from context file, spawn lu-planner
 * to create fix plan, spawn lu-executor to implement fixes, spawn lu-verifier
 * to confirm fixes. Track results with commit hashes and files modified.
 *
 * **Input:** Valid concerns from context file (pr_validate section)
 * **Output:** Populated `pr_fix` section in `/tmp/pr-address-context.json`
 *
 * This is an orchestrating sub-skill — it spawns lu-planner, lu-executor,
 * and lu-verifier via Task().
 *
 * @see .planning/phases/223-anti-skip-pilot/01-PLAN.md Task 6
 */
import { createSkill } from "~/skills/__helpers/create-skill";

import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const prFixConfig: SkillConfig = {
  frontmatter: {
    name: "pr-fix",
    description:
...
export const prFixSkill = createSkill(prFixConfig);
