/**
 * scout-graduate Skill - MuninnDB engram capture for scouting findings.
 *
 * Sub-skill for Step 8 of the scout pipeline (cross-cutting batch). Follows
 * the research-graduator pattern to extract high-value findings from scouting
 * digests and impact documents, scoring them for confidence, actionability, and
 * uniqueness before storing as long-term MuninnDB engrams.
 *
 * @example
 * ```
 * Skill(skill: "scout-graduate", args: "{integrated_slugs} {deferred_slugs} {digest_paths} {impact_paths}")
 * ```
 */
import { createSkill } from "~/skills/__helpers/create-skill";
import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const scoutGraduateConfig: SkillConfig = {
  frontmatter: {
    name: "scout-graduate",
    description:
...
export const scoutGraduateSkill = createSkill(scoutGraduateConfig);
