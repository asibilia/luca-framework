/**
 * phase-research Skill - Conduct comprehensive ecosystem research with
 * parallel specialist agents (v2) or single researcher (v1).
 *
 * Detects workflow.version in .planning/config.json to branch:
 * - v2: Creates research/ directory, writes 00-brief.md, spawns 4 parallel researchers
 * - v1 (default): Single lu-phase-researcher, single RESEARCH.md
 */
...
const phaseResearchConfig: SkillConfig = {
  frontmatter: {
    name: "phase-research",
    description: `Conduct comprehensive ecosystem research with parallel specialist agents (v2) or single researcher (v1).`,
...
3. **Spawn 4 researchers in parallel via Task():**
   ...
   |-------|-------------|-------|
   | lu-architecture-researcher | \`$PHASE_DIR/research/01-architecture-patterns.md\` | System design, patterns, structure |
   | lu-implementation-researcher | \`$PHASE_DIR/research/02-implementation-approaches.md\` | APIs, code patterns, configuration |
