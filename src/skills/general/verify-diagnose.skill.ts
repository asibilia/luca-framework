/**
 * verify-diagnose Sub-Skill — Diagnose UAT failures and create fix plans.
 *
 * Extracts Step 8 from the monolithic verify skill.
 *
 * **Responsibility:** Spawn lu-debugger agents in parallel for each failed
 * UAT test, then spawn lu-planner to create fix plans, then spawn
 * lu-plan-checker to validate those plans. Write results to the shared
 * context file.
 *
 * This sub-skill only runs if UAT issues were found (Path B: orchestrator
 * sends DIAGNOSE_COMPLETE after this completes).
 *
 * **Input:** Phase number and test results (from orchestrator context)
 * **Output:** Populated `verify_diagnose` section in `/tmp/verify-context.json`
 *
 * @see .planning/phases/224-anti-skip-rollout/02-PLAN.md Task 5
 */
import { createSkill } from "~/skills/__helpers/create-skill";

import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const verifyDiagnoseConfig: SkillConfig = {
  frontmatter: {
...
export const verifyDiagnoseSkill = createSkill(verifyDiagnoseConfig);
