/**
 * pr-fetch Sub-Skill — Resolve PR context and fetch all comment types from GitHub.
 *
 * Extracts Steps 0-1 from the monolithic pr-address skill.
 *
 * **Responsibility:** Resolve PR number, fetch review comments, issue comments,
 * reviews, and diff from GitHub. Filter actionable comments. Write results to
 * the shared context file.
 *
 * **Input:** PR number or URL (from Skill() args)
 * **Output:** Populated `pr_fetch` section in `/tmp/pr-address-context.json`
 *
 * This is a leaf skill — it does NOT spawn sub-agents via Task().
 *
 * @see .planning/phases/223-anti-skip-pilot/01-PLAN.md Task 3
 */
import { createSkill } from "~/skills/__helpers/create-skill";

import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const prFetchConfig: SkillConfig = {
  frontmatter: {
    name: "pr-fetch",
    description:
...
export const prFetchSkill = createSkill(prFetchConfig);
