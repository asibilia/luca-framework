/**
 * milestone-finalize Sub-Skill — Commit, tag, and divergent mode advisory.
 *
 * Extracts Steps 8-9 from the monolithic milestone-complete skill.
 *
 * **Responsibility:** Create git tag, final commit, handle divergent mode
 * advisory, and write results to the shared context file.
 *
 * **Input:** Milestone version (from orchestrator context)
 * **Output:** Populated `milestone_finalize` section in `/tmp/milestone-complete-context.json`
 *
 * @see .planning/phases/224-anti-skip-rollout/01-PLAN.md Task 7
 */
import { createSkill } from "~/skills/__helpers/create-skill";

import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const milestoneFinalizeConfig: SkillConfig = {
  frontmatter: {
    name: "milestone-finalize",
    description:
...
export const milestoneFinalizeSkill = createSkill(milestoneFinalizeConfig);
