/**
 * pr-debate Sub-Skill — Handle split verdict debates when validators disagree.
 *
 * Extracts Step 4.5 from the monolithic pr-address skill.
 *
 * **Responsibility:** Read split verdicts from context file, spawn dissenter
 * and majority response agents for each split, build structured debate results.
 *
 * **Input:** Split verdicts from context file (pr_validate section)
 * **Output:** Populated `pr_debate` section in `/tmp/pr-address-context.json`
 *
 * This sub-skill is OPTIONAL (PREMORTEM Constraint #2). The orchestrator
 * sends SKIP_DEBATE if no split verdicts exist, or DEBATE_COMPLETE after
 * this skill runs.
 *
 * @see .planning/phases/223-anti-skip-pilot/01-PLAN.md Task 5
 */
import { createSkill } from "~/skills/__helpers/create-skill";

import type { SkillConfig } from "~/skills/__schemas/skill.schemas";

const prDebateConfig: SkillConfig = {
  frontmatter: {
    name: "pr-debate",
    description:
...
export const prDebateSkill = createSkill(prDebateConfig);
