/**
 * lu-shadow-scanner Agent - Scans repository for AI-session debris and outputs a structured ShadowScanReport.
 */
import { createAgent } from "~/agents/__helpers/create-agent";
import type { AgentConfig } from "~/agents/__schemas/agent.schemas";

// Define the lu-shadow-scanner agent configuration
const luShadowScannerConfig: AgentConfig = {
  frontmatter: {
    name: "lu-shadow-scanner",
    description: `Scans the repository for AI-session debris: orphaned temp scripts, misplaced files, tool artifacts, dead exports, stale planning artifacts, orphaned/misplaced markdown, and repo-root markdown debris. Outputs a structured ShadowScanReport.`,
...
Extract: \`enabled\`, \`denylist_patterns\`, \`known_good_script_dirs\`, \`known_artifact_dirs\`, \`allowlist\`, \`repo_root_markdown_allowlist\`.
...
and never cleaned up (e.g., \`FIRECRAWL_WEB_SEARCH_IMPLEMENTATION.md\`, \`MASTRA_SETUP.md\`, \`PLAN.md\`).
...
**Severity mapping:**
- Filenames in \`SCREAMING_CASE\` (all uppercase with underscores, e.g., \`MASTRA_SETUP.md\`,
  \`FIRECRAWL_WEB_SEARCH_IMPLEMENTATION.md\`): \`medium\` — strong AI-debris signal
...
\`recommended_action: "delete"\`, \`auto_fixable: false\` (user should confirm before deletion).

**SCREAMING_CASE heuristic:** A filename (without extension) matches SCREAMING_CASE if it contains
only uppercase letters A-Z, digits 0-9, underscores, and hyphens, AND contains at least one underscore
(e.g., \`MASTRA_SETUP\`, \`FIRECRAWL_WEB_SEARCH_IMPLEMENTATION\`). Single-word uppercase filenames
like \`PLAN.md\` do NOT match the heuristic — they get \`low\` severity.
</detection_logic>`,
      order: 3,
    },
    {
      title: "muninn_integration",
      content: `<muninn_integration>
...
export const luShadowScannerAgent = createAgent(luShadowScannerConfig);
