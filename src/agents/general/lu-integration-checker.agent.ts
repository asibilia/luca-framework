...
check_export_used() {
  ...
  local imports=$(grep -r "import.*$export_name" "$search_path" \
    ...
    grep -v "$source_domain" | wc -l)

  # Find usage (not just import)
  local uses=$(grep -r "$export_name" "$search_path" \
    --include="*.ts" 2>/dev/null | \
    grep -v "import" | grep -v "$source_domain" | wc -l)
...
check_tier_compliance() {
  ...
  local cross_entity=$(grep -r "from.*~/skills\|from.*~/rules" src/agents/ \
  ...
  local upward=$(grep -r "from.*~/memory\|from.*~/planner\|from.*~/agents\|from.*~/skills" src/shared/ \
    --include="*.ts" 2>/dev/null | wc -l)
  ...
  local t3_imported=$(grep -r "from.*~/compilers\|from.*~/hooks" src/ \
    --include="*.ts" 2>/dev/null | grep -v "src/compilers\|src/hooks" | wc -l)
