/**
 * lu-router-fast Agent - Fast-tier variant of lu-router for TRIVIAL/SIMPLE tasks.
 *
 * Uses haiku model for quick complexity classification. Same purpose as
 * lu-router but optimized for lightweight tasks where deep analysis
 * is unnecessary.
 */
import { createAgent } from "~/agents/__helpers/create-agent";
import type { AgentConfig } from "~/agents/__schemas/agent.schemas";

const luRouterFastConfig: AgentConfig = {
  frontmatter: {
    name: "lu-router-fast",
    description:
...
export const luRouterFastAgent = createAgent(luRouterFastConfig);
