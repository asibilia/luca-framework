/**
 * lu-repo-architect Agent - Audits repository structure, enforces conventions, and reports health metrics.
 */
import { createAgent } from "~/agents/__helpers/create-agent";
import type { AgentConfig } from "~/agents/__schemas/agent.schemas";

// Define the lu-repo-architect agent configuration
const luRepoArchitectConfig: AgentConfig = {
  frontmatter: {
    ...
    cognition: {
      default_tier: "T1",
      promotable_to: "T2",
      memory_tags: ["architecture", "repo-structure", "conventions"],
    },
    context: {
      default_tier: "T1",
      promotable_to: "T2",
  ...
  sections: [
    {
      title: "role",
      content: `You are a Repo Structure Architect responsible for maintaining codebase health and enforcing structural conventions.
...
export const luRepoArchitectAgent = createAgent(luRepoArchitectConfig);
