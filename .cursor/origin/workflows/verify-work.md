...
Wait for user response (plain text, no AskQuestion).
</step>
...
**If issues == 0:** Proceed to `code_quality_review`
...
- Follow @./.cursor/origin/workflows/diagnose-issues.md
...
This step runs after UAT passes (issues == 0) but before presenting "ready to continue".
...
**If no changed files:** Skip to `present_ready`
...
- **dx-advocate**: "Conventions, coding standards, Lodash vs native methods, snake_case keys, single object arguments"
- **code-simplifier**: "DRY violations, duplicated code, unnecessary complexity, dead code"
- **security-auditor**: "Injection vulnerabilities, auth patterns, input validation, secrets exposure"

**Merge findings:**

Combine all issues from all reviewers. Deduplicate by file:line.
...
- **CRITICAL issues found:** Proceed to `plan_quality_fixes`
- **HIGH/MEDIUM only:** Proceed to `present_quality_warnings`
- **Clean or LOW only:** Proceed to `present_phase_complete`
</step>

<step name="present_quality_warnings">
**Present quality warnings and offer options:**
...
|----------|-------|----------|
| HIGH | {N} | {first HIGH issue} |
| MEDIUM | {N} | {first MEDIUM issue} |
...
- **1 (Fix now):** Proceed to `plan_quality_fixes`
- **2 (Continue):** Proceed to `present_phase_complete`
- **3 (Review details):** Display full findings, then re-present options
</step>

<step name="plan_quality_fixes">
**Plan fixes for quality issues:**
...
<instructions>
Create a plan to fix the quality issues identified by code reviewers.

For each issue:

1. Reference the specific file:line
2. Include the suggested fix from reviewers
3. Group related fixes into single tasks

Output: A PLAN.md with tasks to fix each issue.
</instructions>
...
<step name="present_phase_complete">
**Present phase completion with next steps:**

This step runs when:
- UAT passes AND code quality review is clean (or LOW only)
- UAT passes AND user chose to continue despite HIGH/MEDIUM warnings

Determine next phase and milestone status:

```bash
# Check if more phases remain
CURRENT_PHASE={phase_number}
NEXT_PHASE=$((CURRENT_PHASE + 1))
NEXT_PHASE_DIR=$(ls -d .planning/phases/$(printf "%02d" $NEXT_PHASE)-* 2>/dev/null | head -1)
```
