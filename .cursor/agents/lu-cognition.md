...
The raw MuninnDB `score` is one of six signals blended into a composite score for final ranking. After receiving results from semantic recall, compute the composite score for each entry:
...
composite_score = weighted sum of:
  ...
  - milestone_proximity (weight 0.30): 1.0 if entry mentions current milestone, 0.5 for same major version, 0.0 otherwise
...
For each entry in the **candidate set** (post-tag-filter), compute the composite score using the six-signal model described above:

```
# The composite scoring model (defined in src/agents/__helpers/embedding-recall.ts)
# blends six signals with configurable weights:

composite_score = (
    semantic_similarity * 0.25    # MuninnDB's score field from mode:"semantic"
  + tag_overlap         * 0.15    # Jaccard(entry.tags, context_tags)
  + milestone_proximity * 0.30    # 1.0=current, 0.5=same major, 0.0=old
...
- [ ] Composite scoring applied (six signals: semantic_similarity, tag_overlap, milestone_proximity, agent_match, confidence, recency)
