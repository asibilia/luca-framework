...
**See @./.cursor/origin/references/checkpoints.md**
...
**0. Pre-commit checklist (CRITICAL):**

Before any commit that modifies `apps/` or `packages-ui/`:

```bash
# Check if changeset exists for this work
ls .changeset/*.md
```

**If no changeset exists:**

1. **Check if this is the first changeset** (no other .md files in .changeset/ besides README.md):

   - **STOP and prompt the user** to run `bun changeset` interactively
   - This ensures proper config.json and pre.json files are created
   - Do NOT create the changeset file manually for the first one

2. **If other changesets already exist**, you may create one manually:
...
**Git Safety - NEVER force push:**

- No `git push --force` or `--force-with-lease`
- No amending commits that have been pushed
- If you need to add changes after pushing, create a new commit
...
Format: `{type}({phase}-{plan}): #{issue} {task-name-or-description}`

The `#{issue}` is the GitHub issue number this work is tied to.

```bash
git commit -m "{type}({phase}-{plan}): #{issue} {concise task description}
...
**Use template from:** @./.cursor/origin/templates/summary.md
...
- [ ] Changeset exists (if `apps/` or `packages-ui/` modified)
- [ ] Each task committed individually with proper format
- [ ] No force pushes used (new commits only, no amending pushed commits)
