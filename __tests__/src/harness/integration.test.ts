...
describe('harness integration', () => {
  ...
  test('harness-verification rule can be instantiated', () => {
    const RuleClass = ruleRegistry['harness-verification'];
  ...
  test('template config has build disabled', async () => {
    ...
    const configFile = Bun.file(templateConfigPath);
    const config = await configFile.json();
    const buildCheck = config.harness.checks.find((c: { name: string }) => c.name === 'build');
  ...
  test('harness-verification rule output files exist', async () => {
    const cursorRule = Bun.file(path.join(PROJECT_DIR, '.cursor/rules/harness-verification.mdc'));
    const claudeRule = Bun.file(path.join(PROJECT_DIR, '.claude/rules/harness-verification.md'));
