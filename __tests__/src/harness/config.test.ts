...
describe("harness config loading", () => {
  ...
  test("default config enables test and typecheck", () => {
    const testCheck = DEFAULT_HARNESS_CONFIG.checks.find(
      (c) => c.name === "test",
    );
    const typecheckCheck = DEFAULT_HARNESS_CONFIG.checks.find(
      (c) => c.name === "typecheck",
    );
  ...
  test("default config disables lint", () => {
    const lintCheck = DEFAULT_HARNESS_CONFIG.checks.find(
      (c) => c.name === "lint",
    );
  ...
  test("falls back to defaults when no harness section exists", async () => {
    ...
    expect(config.maxFixIterations).toBe(
      DEFAULT_HARNESS_CONFIG.maxFixIterations,
    );
  });
});
