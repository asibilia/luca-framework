...
import { mkdtemp as _mkdtemp, rm as _rm, writeFile as _writeFile, mkdir as _mkdir, readFile as _readFile } from 'fs/promises';
...
const { configValidationCheck } = await import(
  '../../../../../../../packages/luca-framework/src/utils/doctor/checks/config-validation'
);
...
async function setupTempProject(files: Record<string, string>): Promise<string> {
