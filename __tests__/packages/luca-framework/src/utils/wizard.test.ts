...
describe('createConfigFromArgs', () => {
  test('returns defaults when given empty args', async () => {
    const { createConfigFromArgs } = await import(
      '../../../../../packages/luca-framework/src/utils/wizard'
    );
...
describe('loadConfigFromFile', () => {
  ...
  test('throws for non-existent file', async () => {
    ...
    expect(loadConfigFromFile('/tmp/nonexistent-luca-config-12345.json')).rejects.toThrow();
  });
});
