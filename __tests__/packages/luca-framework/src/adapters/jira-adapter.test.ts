...
import type {
  WorkTicketType,
  WorkTicketPriority,
} from "../../../../../packages/luca-framework/src/contracts/work-tracker";
...
const adfDescription = {
  ...
  content: [
    {
      type: "paragraph",
      content: [{ type: "text", text: "As a user I want to do X" }],
    },
    {
      type: "paragraph",
      content: [{ type: "text", text: "so that Y" }],
    },
  ],
};
...
describe("JiraAdapter", () => {
  ...
  describe("getTicket", () => {
    test("returns ticket for valid issue with ADF description", async () => {
      ...
      if (result.success) {
        ...
        expect(result.data.url).toBe(
          "https://test.atlassian.net/browse/PROJ-1234",
        );
      }
    });
    ...
    test("sends correct auth headers", async () => {
      ...
      const expectedCredentials = Buffer.from(
        "test@example.com:test-api-token",
      ).toString("base64");
  ...
  describe("mapJiraType (via getTicket)", () => {
    const testTypeMapping = async (
      jiraType: string | undefined,
      expectedType: WorkTicketType,
    ) => {
      const issue = {
        ...fullJiraIssueResponse,
        fields: {
    ...
    test("maps Story to story", async () =>
      await testTypeMapping("Story", "story"));
    test("maps Task to task", async () =>
      await testTypeMapping("Task", "task"));
    test("maps Epic to epic", async () =>
      await testTypeMapping("Epic", "epic"));
    test("maps Sub-task to subtask", async () =>
      await testTypeMapping("Sub-task", "subtask"));
    test("defaults to task for unknown type", async () =>
      await testTypeMapping("Custom Type", "task"));
    test("defaults to task when issuetype is undefined", async () =>
      await testTypeMapping(undefined, "task"));
  });
  ...
  describe("mapJiraPriority (via getTicket)", () => {
    const testPriorityMapping = async (
      ...
      expectedPriority: WorkTicketPriority,
    ) => {
      const issue = {
        ...fullJiraIssueResponse,
        fields: {
    ...
    test("maps Highest to highest", async () =>
      await testPriorityMapping("Highest", "highest"));
    test("maps High to high", async () =>
      await testPriorityMapping("High", "high"));
    test("maps Medium to medium", async () =>
      await testPriorityMapping("Medium", "medium"));
    test("maps Low to low", async () =>
      await testPriorityMapping("Low", "low"));
    test("maps Lowest to lowest", async () =>
      await testPriorityMapping("Lowest", "lowest"));
    test("defaults to medium for unknown priority", async () =>
      await testPriorityMapping("Blocker", "medium"));
    test("defaults to medium when priority is undefined", async () =>
      await testPriorityMapping(undefined, "medium"));
  });
