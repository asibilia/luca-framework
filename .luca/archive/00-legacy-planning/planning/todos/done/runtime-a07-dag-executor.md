---
title: "Runtime A07: Implement DAG executor with wave execution"
area: workflow
created: 2026-03-24
source: docs/runtime-architecture/dag-workflow-engine.md
depends_on: [A01, A02, A05, A06, A08]
phase: runtime-a
estimated_files: 2
---
...
Implement the DAG executor — the most complex component in Phase A. The executor runs a validated DAG wave-by-wave through an adapter interface, handling guards, retries with backoff, per-step timeouts via AbortController, checkpoint persistence after each wave, and fail-isolated parallel execution via `Promise.allSettled`. This is the integration point where all other Phase A components come together.

The executor does NOT manage its own workflow state independently. Per risk-analysis.md Risk 13 (Option A recommendation), it is designed as a pure execution engine that receives a DAG, an adapter, and a context, and returns an `ExecutionResult`. XState state machine integration (sending events like `CLASSIFY_COMPLETE`) is deferred to Phase B when the adapter layer wires the executor to the existing state machine.

## Task

### Files to Create

#### `src/workflow/__helpers/dag-executor.ts`
...
Replace the `// Added by A07` placeholder comment under `DAG Executor` with actual exports:
...
- [ ] Guard returning false skips the step with `status: "skipped"`
- [ ] Guard throwing an exception skips the step (not crashes)
- [ ] Step timeout produces `status: "timeout"` result
- [ ] Retry exhaustion produces `status: "failed"` with `retryCount` equal to max-1
- [ ] `Promise.allSettled` is used (not `Promise.all`) — parallel step failure does not cancel siblings
- [ ] Checkpoint is persisted after each wave when `persistCheckpoints: true`
...
- Depends on: A01 (structure), A02 (schemas), A05 (sorter), A06 (validator), A08 (serializer)
- A08 (serializer) must be completed first because the executor calls `saveCheckpoint` and `clearCheckpoint` directly
- The executor passes `__abortSignal` in the context so adapters can respect timeout cancellation
- XState state machine integration is deferred to Phase B. The executor is a pure execution engine.
- Schema validation defaults to "warn" mode per Risk 11 recommendation
- The `input` gathered for each step is a map of `{ depStepId: depStepOutput }` — the step's inputSchema should validate this structure
- Backoff delays: none=0ms, linear=attempt*1000ms, exponential=2^attempt*500ms
