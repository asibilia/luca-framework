...
function synthesizeSummary(entry: Record<string, unknown>): string {
    ...
    if (eventType.includes('PHASE') && eventType !== 'field_set') {
        const phaseId = get(eventData, 'phase_id', null) as
            | number
            | string
            | null
        if (phaseId != null) {
            ...
            if (eventType === 'PHASE_COMPLETE')
                return `Phase ${phaseId} completed`
    ...
    if (eventType === 'CHECKS_COMPLETE') {
        ...
        if (status) {
            const errorCount = get(eventData, 'total_errors', null) as
                | number
                | null
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (stateRes && stateRes.ok) {
                const stateJson = (await stateRes.json()) as Record<
                    string,
                    unknown
                >
                setState(stateJson)
