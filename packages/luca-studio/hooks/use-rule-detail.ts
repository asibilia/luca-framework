...
import { ruleDraftAtom, ruleHistoryAtom } from "~/stores/entity-atoms";
...
type UseRuleDetailReturn = {
  /** Full rule detail from the API, or null if not yet loaded. */
  ...
  etag: string | null;
  /** Manually refetch the rule detail. */
  refresh: () => void;
};
 ...
 * ```
 */
export function useRuleDetail(name: string | null): UseRuleDetailReturn {
  const [detail, setDetail] = useState<EntityDetail | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [etag, setEtag] = useState<string | null>(null);
  const setDraft = useSetAtom(ruleDraftAtom(name ?? "__noop__"));
  const resetHistory = useSetAtom(ruleHistoryAtom(name ?? "__noop__"));
  ...
  const fetchDetail = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/entities/rules/${encodeURIComponent(name)}`,
      );
      if (!res.ok) {
      ...
      // Only apply if we haven't switched rules mid-flight
      ...
      if (nameRef.current === name) {
        const message =
          err instanceof Error ? err.message : "Failed to load rule detail";
        setError(message);
      }
    } finally {
      if (nameRef.current === name) {
