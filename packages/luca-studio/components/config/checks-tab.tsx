...
/** Shape of a single verification check entry. */
type CheckEntry = {
  name: string;
  command: string;
  enabled: boolean;
...
/**
 * Checks tab displaying check type toggles, command overrides, and
 ...
 * global checks settings (enabled, maxFixIterations, failFast).
 *
...
export function ChecksTab() {
  ...
  const checks = useMemo(() => {
    return get(harness, "checks", []) as CheckEntry[];
  }, [harness]);
  ...
  const updateCheck = useCallback(
    (idx: number, field: keyof CheckEntry, value: unknown) => {
      setConfig((prev) => {
        const current = { ...(prev ?? {}) };
        const currentHarness = {
          ...(get(current, "harness", {}) as Record<string, unknown>),
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between rounded-md border px-3 py-2">
          <div>
            <span className="text-sm font-medium">Checks Enabled</span>
            <p className="text-xs text-muted-foreground">
              Run verification checks at phase boundaries
            </p>
          </div>
          <Switch
