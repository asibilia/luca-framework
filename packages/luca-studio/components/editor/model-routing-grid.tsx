...
function detectPreset(value: RoutingValue): string | null {
  for (const [name, preset] of Object.entries(ROUTING_PRESETS)) {
    const matches = COMPLEXITY_KEYS.every((key) => preset[key] === value[key]);
    ...
    <div className={cn("inline-flex flex-col gap-1.5", className)}>
      ...
      {presetName && (
        <span className="text-xs font-medium text-muted-foreground">
          Preset: <span className="text-foreground">{presetName}</span>
        </span>
      )}
        ...
        {COMPLEXITY_KEYS.map((level) =>
          readOnly ? (
            ...
            <TierSelect
              key={level}
              value={value[level]}
              onValueChange={(tier) => handleTierChange(level, tier)}
            />
          ),
        )}
      </div>
