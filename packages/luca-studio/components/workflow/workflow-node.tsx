...
export function WorkflowNode({ data, selected }: NodeProps) {
  ...
  const borderClass = DOMAIN_BORDER[nodeData.domain] ?? DOMAIN_BORDER.execution;
    ...
    >
      ...
      <div className="px-3 py-1.5 space-y-0.5">
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>
            Model tier:{" "}
            <span className="text-foreground">{nodeData.modelTier}</span>
          </span>
          <span>
            Agents:{" "}
            <span className="text-foreground">{nodeData.agentCount}</span>
          </span>
        </div>
