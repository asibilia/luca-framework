...
- 9bd779b: Muninn cleanup: tree-walk write-surface for the todo backlog and brain tree.
...
- 8952fa3: Milestone v13.1.0 — Telemetry Impact & Attribution. Close the gap between activity-capture (which Luca already did well) and impact-measurement, across four phases.
...
- 479e2bc: Stage-gate: classify desktop viewer-open launchers (`open`, `xdg-open`,
...
- 012942a: Stage-gate: allow inert ephemeral scratch writes in any pipeline step.
  ...
  `/var` system-dir rule hard-denied in _every_ phase.
  ...
  - **OS temp dirs** — the universal `/tmp` and `/private/tmp`, plus the
    ...
    `/var/folders/…/T` is recognised _before_ the `/var` denial.
...
- 3074d73: Adopt PAI v5.0.0 review learnings across the planning, verification, learning, and governance surfaces (v13.0.0-pai-learnings milestone, REQ-01…REQ-10).
