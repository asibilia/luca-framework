 ...
 *   - Errors thrown by a rule become an `executionError` so one bad
 *     rule cannot crash the run.
 *
 * Reporting:
 ...
 *
 * Ported from luca-mastracode `rule-engine/runner.ts`. Two changes from
 * the original: the `typescript` type import uses the namespace form
 * (`import type * as ts` — luca-core's tsconfig has no esModuleInterop),
 * and the unused `_formatRelative` export was dropped (dead code; its
 * "used below" comment was stale).
 */
 ...
 * We don't use `instanceof` because rule packs may import `defineRule`
 * from a different module instance after dynamic import; we duck-type.
 */
 ...
 * paths. Uses `Bun.Glob` (bundled with the Bun runtime).
 */
...
/**
 * Build a hybrid RuleFile with raw content + lazy AST. AST parses are
 * cached on the RuleFile instance, so multiple rules processing the same
 * file pay the parse cost only once.
 */
...
export function runRules(opts: {
    ...
    for (const rule of rules) {
        ...
        for (const relPath of candidates) {
            // `scope: 'repo'` produces a single sentinel candidate (`''`).
            // For repo-scoped rules we synthesize a RuleFile pointing at the
            // repo root rather than trying to read it as a file.
            const file =
                relPath === ''
                    ? ({
                          path: '',
