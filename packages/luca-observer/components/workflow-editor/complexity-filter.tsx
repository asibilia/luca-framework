  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      if (e.key === "ArrowRight" || e.key === "ArrowDown") {
        e.preventDefault();
        nextIndex = currentIndex < LEVELS.length - 1 ? currentIndex + 1 : 0;
      } else if (e.key === "ArrowLeft" || e.key === "ArrowUp") {
        e.preventDefault();
        nextIndex = currentIndex > 0 ? currentIndex - 1 : LEVELS.length - 1;
      }
      ...
      {LEVELS.map((level) => {
        ...
        return (
          <button
            ...
            tabIndex={isSelected || (!value && level === LEVELS[0]) ? 0 : -1}
