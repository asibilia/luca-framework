...
export const GraphCanvas = forwardRef<GraphCanvasHandle, GraphCanvasProps>(
  function GraphCanvas(
    ...
    const nodeCanvasObject = useCallback(
      (raw: NodeObject, ctx: CanvasRenderingContext2D, globalScale: number) => {
        const node = raw as FGNode;
        const x = node.x ?? 0;
        const y = node.y ?? 0;
        ...
        if (node.is_cluster) {
          ...
          if (globalScale >= 0.5) {
            ...
            ctx.fillText(node.name, x, y + radius + labelFontSize + 2);
    ...
    const nodePointerAreaPaint = useCallback(
      (raw: NodeObject, paintColor: string, ctx: CanvasRenderingContext2D) => {
        const node = raw as FGNode;
        const x = node.x ?? 0;
        const y = node.y ?? 0;
        const radius = node.is_cluster ? Math.sqrt(node.child_count) * 4 : 6; // slightly larger than visual for easier clicking
