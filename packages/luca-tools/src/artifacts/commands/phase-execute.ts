/**
 * phase-execute slash command — Drive the "execute" pipeline step — implement the phase plan with per-task commits and verification.
 *
 * Ported from ~/.claude/commands/phase-execute.md (user copy canonical) (E-6).
 */
import { defineCommand } from '../../define/command.ts'

const BODY = `# /phase-execute
...
export const phaseExecuteCommand = defineCommand({
    name: 'phase-execute',
    description:
        'Drive the "execute" pipeline step — implement the phase plan with per-task commits and verification.',
    body: BODY,
})
