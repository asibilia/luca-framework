 ...
 * instructions (via `shared-prefix.ts`).
 *
 * Single source of truth: change here once; both injection paths pick it up.
 *
 * Ported from luca-mastracode/src/memory-tier-discipline.ts.
 *
 * Token budget: enforced ceiling is 1600 chars (~400 tokens). Aim to keep
 * under ~800 chars for headroom. Every char here is multiplied across the
 * 7 subagents and 10 mode-agents that interpolate it.
 */
