...
You cannot reach MuninnDB. Return the HIGH/MEDIUM-confidence learnings as the machine-parseable block below so the orchestrator (which HAS MuninnDB access) can persist them via \`mcp__muninn__muninn_remember_batch\` and dedup against existing memories. NOTE the dedup is **best-effort**: MuninnDB has no concept lookup, so the orchestrator can only recall semantically before writing, and recall is weak/empty on a vault with no embedder — a near-duplicate may slip through. Prefer a distinctive, stable \`concept:\` slug per learning (so a later \`muninn_consolidate\` can merge dupes) over assuming the pre-write dedup is exhaustive. Annotate each with its target vault per the routing rule, using these EXACT \`vault:\` values:
