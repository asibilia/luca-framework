...
function loadInstructions(): string {
  return readFileSync(join(__dirname, '..', 'instructions', 'research.md'), 'utf-8');
}
 ...
 */
export function buildResearchInstructions(_harnessState?: Record<string, unknown>): string {
 ...
 */
export function resolveResearchModel(_harnessState?: Record<string, unknown>): string {
  const state = readLucaState();
  return resolveModel({
