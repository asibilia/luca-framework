...
function loadInstructions(): string {
  return readFileSync(join(__dirname, '..', 'instructions', 'architect.md'), 'utf-8');
}
 ...
 */
export function buildArchitectInstructions(_harnessState?: Record<string, unknown>): string {
 ...
 */
export function resolveArchitectModel(_harnessState?: Record<string, unknown>): string {
  const state = readLucaState();
  return resolveModel({
