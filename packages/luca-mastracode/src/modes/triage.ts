...
function loadInstructions(): string {
  return readFileSync(join(__dirname, '..', 'instructions', 'triage.md'), 'utf-8');
}
 ...
 */
export function buildTriageInstructions(_harnessState?: Record<string, unknown>): string {
...
export function resolveTriageModel(_harnessState?: Record<string, unknown>): string {
  ...
  return resolveModel({
    ...
    complexity: state.complexity as 'TRIVIAL' | 'SIMPLE' | 'MODERATE' | 'COMPLEX' | 'CRITICAL',
    profile: (state.profile as 'budget' | 'balanced' | 'quality') ?? 'balanced',
  });
}
