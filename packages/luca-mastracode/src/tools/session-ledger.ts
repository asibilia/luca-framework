...
export const sessionLedgerTool = createTool({
  ...
  inputSchema: z.object({
    ...
    eventType: z.string().optional().describe('Event type to filter by (for filter action)'),
    limit: z.number().optional().describe('Max entries to return (for routing-history, default 20)'),
  }),
  execute: async (inputData) => {
    ...
    switch (action) {
      ...
      case 'filter': {
        if (!eventType) return { success: false, message: 'eventType required for filter' };
