...
export const pipelineLockTool = createTool({
  ...
  inputSchema: z.object({
    ...
    pipelineStep: z.string().optional().describe('Current pipeline step (for update)'),
    phaseStep: z.string().optional().describe('Current phase sub-step (for update)'),
  }),
    ...
    switch (action) {
      case 'status': {
        if (!existsSync(lockPath)) {
          return { status: 'clear' as const, message: 'No active lock' };
        }
        try {
          ...
          try {
            ...
            return { status: 'live' as const, message: `Active lock held by PID ${lock.pid}`, lock };
          } catch {
            return { status: 'stale' as const, message: `Stale lock from PID ${lock.pid}`, lock };
          }
        } catch {
          return { status: 'stale' as const, message: 'Corrupt lock file' };
        }
      }
      case 'acquire': {
        const lock: LockInfo = {
      ...
      case 'update': {
        if (!existsSync(lockPath)) {
          return { status: 'clear' as const, message: 'No lock to update' };
        }
        const lock: LockInfo = JSON.parse(readFileSync(lockPath, 'utf-8'));
      ...
      case 'recover': {
        if (!existsSync(lockPath)) {
          return { status: 'clear' as const, message: 'No lock found — no recovery needed' };
        }
        let lock: LockInfo;
        try {
          ...
          unlinkSync(lockPath);
          return { status: 'clear' as const, message: 'Corrupt lock file removed — start fresh' };
        }
      ...
      default:
        return { status: 'clear' as const, message: `Unknown action: ${action}` };
    }
  },
});
