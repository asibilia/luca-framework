...
export const repoCleanupTool = createTool({
  ...
  inputSchema: z.object({
    ...
    scan_mode: z
      ...
      .optional()
      .describe("Explicit scan mode (for scan action). Auto-determined if omitted."),
    raw_output: z
      .string()
      .optional()
      .describe("Raw shadow-scanner subagent output (for parse-report action)"),
    file_path: z
      .string()
      .optional()
    ...
    target_path: z
      .string()
      .optional()
      .describe("Destination path for move action (for apply-fix action)"),
  }),
  execute: async (input) => {
    const { action, scan_mode, raw_output, file_path, recommended_action, target_path } = input;
    ...
    switch (action) {
      case "scan": {
        ...
        if (!config.enabled) {
          return {
            status: "disabled",
            message: "Shadow debt scanning is disabled in .planning/config.json",
          };
        }
      ...
      case "parse-report": {
        ...
        const jsonMatch = raw_output.match(/```json\s*\n([\s\S]*?)\n\s*```/);
        if (jsonMatch?.[1]) {
          jsonStr = jsonMatch[1];
        } else {
          // Try to find a raw JSON object with scan_mode key
          const rawMatch = raw_output.match(/(\{[\s\S]*"scan_mode"[\s\S]*\})\s*$/);
          if (rawMatch?.[1]) {
            jsonStr = rawMatch[1];
          }
        }
      ...
      case "apply-fix": {
        if (!file_path || !recommended_action) {
          return { error: "file_path and recommended_action are required for apply-fix" };
        }
        ...
        switch (recommended_action) {
          case "delete": {
            if (!existsSync(fullPath)) {
              return { status: "skipped", message: `File not found: ${file_path}` };
            }
            unlinkSync(fullPath);
            return { status: "applied", action: "delete", file_path };
          }
          ...
          case "move": {
            ...
            return { status: "applied", action: "move", file_path, target_path };
          }

          case "gitignore": {
            const gitignorePath = join(process.cwd(), ".gitignore");
            const entry = file_path.endsWith("/") ? file_path : `${file_path}\n`;
            const existing = existsSync(gitignorePath)
              ? readFileSync(gitignorePath, "utf-8")
              : "";
            if (existing.includes(file_path)) {
              return { status: "skipped", message: `Already in .gitignore: ${file_path}` };
            }
            const newline = existing.endsWith("\n") ? "" : "\n";
            appendFileSync(gitignorePath, `${newline}${entry}`);
            return { status: "applied", action: "gitignore", file_path };
          }

          default:
            return { error: `Unknown action: ${recommended_action}` };
        }
      }
