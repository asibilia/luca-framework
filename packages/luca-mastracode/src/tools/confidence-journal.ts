...
export const confidenceJournalTool = createTool({
  ...
  description: 'Track execution-time decision confidence. Log entries when the executor encounters ambiguity, makes on-the-fly decisions, or lacks sufficient plan detail. Produces a reviewable `.planning/CONFIDENCE-JOURNAL.md` that highlights blocks needing human re-review.',
  inputSchema: z.object({
    action: z.enum(['log', 'read', 'summary', 'render']).describe(
      'log: record a confidence entry | read: get all entries | summary: aggregate stats | render: regenerate Markdown file'
    ),
    entry: z.object({
      phase: z.string().describe('Phase name from PLAN.md / ROADMAP.md'),
      wave: z.number().describe('Wave number within the phase'),
      task: z.string().describe('Task ID or description from PLAN.md'),
      ...
      reasoning: z.string().describe('Why this choice was made over alternatives'),
      risk: z.string().describe('What could go wrong if this was the wrong call'),
      files: z.array(z.string()).describe('Affected file paths'),
      reviewHint: z.string().optional().describe('Suggested focus area for human reviewer'),
    }).optional().describe('Confidence entry to log (required for "log" action)'),
  }),
  execute: async (inputData) => {
    ...
    switch (action) {
      case 'log': {
        if (!entry) {
          return { success: false, message: 'entry is required for log action' };
        }
      ...
      case 'summary': {
        ...
        const lowWarning = summary.low > 0
