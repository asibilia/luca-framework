...
export const manageRoadmapTool = createTool({
  ...
  inputSchema: z.object({
    ...
    phases: z.array(z.object({
      ...
      status: z.enum(['pending', 'in-progress', 'complete', 'skipped', 'blocked']).default('pending'),
      businessValue: z.number().optional().describe('Business value for WSJF scoring (1-10)'),
      timeCriticality: z.number().optional().describe('Time criticality for WSJF scoring (1-10)'),
      effort: z.number().optional().describe('Effort estimate for WSJF scoring (1-10)'),
    })).optional().describe('Phase definitions (required for create action)'),
    phaseName: z.string().optional().describe('Phase name (for update-status action)'),
    newStatus: z.enum(['pending', 'in-progress', 'complete', 'skipped', 'blocked']).optional().describe('New status (for update-status action)'),
    maxPhases: z.number().optional().describe('Maximum phases to include in execution order'),
  }),
    ...
    switch (action) {
      case 'create': {
        if (!phases || phases.length === 0) {
          return { success: false, message: 'No phases provided for creation', phases: [] };
        }
      ...
      case 'compute-order': {
        ...
        return { success: true, message: `Computed execution order: ${limited.length} phases`, executionOrder: limited };
      }
      case 'update-status': {
