...
export const verificationResultTool = createTool({
  ...
  inputSchema: z.object({
    ...
    }).optional().describe('Verification result to write (required for write action)'),
  }),
  execute: async (inputData) => {
    ...
    switch (action) {
      case 'write': {
        if (!result) {
          return { success: false, message: 'result is required for write action' };
        }
        writeVerificationResult({
          ...result,
      ...
      case 'read': {
        ...
        if (!latest) {
          return { success: false, message: 'No verification result found' };
        }
        return {
          success: true,
      ...
      case 'read-history': {
        ...
        return {
          ...
          history: history as unknown as Array<Record<string, unknown>>,
        };
      }
      case 'aggregate': {
        const all = readVerificationHistory();
        if (all.length === 0) {
          return { success: false, message: 'No verification history to aggregate' };
        }
