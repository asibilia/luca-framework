...
3. **Hang-timeout — fast-fail on slow subagents.** The Mastra harness has no per-subagent `timeoutMs` API, so the orchestrator enforces timeout in prose: for each spawn capture `const start = Date.now()`. After all returns, compute `elapsed = Date.now() - start` per subagent. If `elapsed > 60_000` (60s wall-clock) treat that subagent as a timeout: emit its `record-subagent` complete with `success: false, outcome: "timeout", inputTokens: null, outputTokens: null` and proceed with the remaining results. Do NOT block waiting for hung subagents — the synthesizer must produce partial findings when at least 3/5 dimensions returned. If fewer than 3/5 returned successfully, mark the wave STALLED and escalate.
