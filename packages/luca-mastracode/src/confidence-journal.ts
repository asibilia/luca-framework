 ...
 */
export function appendConfidenceEntry(entry: Omit<ConfidenceEntry, 'timestamp'>): ConfidenceEntry {
  ensurePlanningDir();
  const full: ConfidenceEntry = { ...entry, timestamp: new Date().toISOString() };
...
export function renderConfidenceJournalMarkdown(): string {
  ...
  if (summary.low > 0) {
    lines.push(`⚠️ **${summary.low} low-confidence decision${summary.low > 1 ? 's' : ''} require${summary.low === 1 ? 's' : ''} human review** — see entries marked 🔴 below.`);
    lines.push('');
  }
