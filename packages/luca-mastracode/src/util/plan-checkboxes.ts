 ...
 *
 * The pipeline's executor stage commits work but does NOT mark `- [ ]` tasks
 * as `- [x]` in `PLAN.md`. That left the planning artifact perpetually
 * out-of-sync with reality: even after a phase passed verification + review
 * and the work shipped, `PLAN.md` still claimed everything was unstarted.
 *
 * Rather than ask the executor to tick its own boxes (low integrity — the
 * executor is the *claimant*, not the verifier), we tick boxes from the
 * `complete-phase` action AFTER the diff guard and verification guard have
 * already passed. By that point the phase is independently attested:
 *
 *   1. `phase-diff` proved real file changes / commits exist.
 *   2. A PASS `verification-result.json` exists for the current wave.
 *
 * Only then does this module flip checkboxes for the matching phase section
 * in PLAN.md. The result: PLAN.md becomes a faithful audit trail, and any
 * unchecked task remaining post-completion is a meaningful signal (either a
 * heading mismatch or a deliberately deferred task).
 *
 ...
 * blocks `complete-phase`. The caller logs the outcome to the session
 ...
 * - `success: true, tickedCount === 0` — section found but already complete
 *   (or genuinely had no checkbox lines). Advisory note in `reason`.
 * - `success: false`                   — couldn't tick. `reason` explains
 *   (file missing, heading not found, write failure). Caller decides
 *   whether to surface as a warning.
 */
...
/**
 * Match an `## Phase N: ...` or `### Phase N: ...` style heading whose text
 * (after the `Phase N:` prefix) equals `phaseName`. Also matches a bare
 * `## phaseName` heading for cases where the executor names a phase
 * directly. Comparison is case-insensitive and tolerates trailing
 * whitespace.
 */
 ...
 * @param phaseName Phase name to match. Compared case-insensitively against
 *                  heading text, with `Phase N:` prefix stripping.
 */
export function tickPhaseTasks(
    ...
    // file had a trailing newline so we don't accidentally add/strip one.
