...
async function main() {
  ...
  const READ_ONLY_DISABLED: Record<string, { name: string; enabled: false }> = {
    [WORKSPACE_TOOLS.FILESYSTEM.WRITE_FILE]: { name: "write_file", enabled: false },
    [WORKSPACE_TOOLS.FILESYSTEM.EDIT_FILE]: { name: "string_replace_lsp", enabled: false },
    [WORKSPACE_TOOLS.FILESYSTEM.AST_EDIT]: { name: "ast_smart_edit", enabled: false },
    ...
    [WORKSPACE_TOOLS.SANDBOX.EXECUTE_COMMAND]: { name: "execute_command", enabled: false },
  ...
  harness.subscribe(async (event) => {
    ...
    if (READ_ONLY_MODES.has(event.modeId)) {
      await harness.setState({
        permissionRules: {
          categories: { read: "allow", edit: "deny", execute: "deny", mcp: "allow" },
