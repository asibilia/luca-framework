  ...
  async run(): Promise<CheckResult> {
    ...
    try {
      ...
      if (config.branding && typeof config.branding === 'object') {
        ...
        const missingBrandingFields = requiredBrandingFields.filter(f => !branding[f]);
      ...
      const isJsonEscapeError = errorMessage.includes('escape') || errorMessage.includes('token') || errorMessage.includes('Unexpected');
      return {
        name: this.name,
        status: 'fail',
