...
**Branding:** Read `.planning/config.json` branding section at session start. Use `/{commandPrefix}` instead of `<%= branding.commandSlash %>` and `{frameworkName}` instead of `<%= branding.frameworkName %>` in ALL user-facing output. If config is missing, fall back to defaults (`<%= branding.commandSlash %>`, `<%= branding.frameworkName %>`).

The single entry point for all <%= branding.frameworkName %> workflows. This is a **thin orchestrator** that delegates ALL work to sub-skills via Skill() calls. It contains ZERO inline logic.
...
|-----------|---------------|-----------------|
| <%= branding.commandPrefix %>-route | Parse request, git context, cognition, classify complexity | idle |
| <%= branding.commandPrefix %>-configure | Read config, apply overrides, pre-flight validation | routed |
| <%= branding.commandPrefix %>-backlog | Backlog scan, WSJF scoring, roadmap revision (OPTIONAL) | configured |
| <%= branding.commandPrefix %>-phase-loop | Phase execution loop, milestone gate, session summary | scanned OR configured |
...
### Step 2: Route (<%= branding.commandPrefix %>-route)

```
Skill(skill: "<%= branding.commandPrefix %>-route", args: "{user_request}")
```
...
1. Spawn <%= branding.commandPrefix %>-verifier via Task()
2. Spawn <%= branding.commandPrefix %>-learner via Task()
...
### Step 3: Configure (<%= branding.commandPrefix %>-configure)

```
Skill(skill: "<%= branding.commandPrefix %>-configure")
```
...
### Step 4: Backlog (<%= branding.commandPrefix %>-backlog) — CONDITIONAL
...
```
Skill(skill: "<%= branding.commandPrefix %>-backlog")
```
...
### Step 5: Execute Phase Loop (<%= branding.commandPrefix %>-phase-loop)
...
```
Skill(skill: "<%= branding.commandPrefix %>-phase-loop")
```
...
The lu session is complete. <%= branding.commandPrefix %>-phase-loop handles the session summary, state updates, and final commit internally.