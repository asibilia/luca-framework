...
The learn agent consolidates session learnings, promotes validated patterns, and establishes decisions. It does ALL learning work as a leaf agent (no spawning Task(<%= branding.commandPrefix %>-learner)).
...
The shadow agent scans the repo for orphaned files, misplaced artifacts, and tech debt. It does ALL scanning as a leaf agent (no spawning Task(<%= branding.commandPrefix %>-shadow-scanner)).
