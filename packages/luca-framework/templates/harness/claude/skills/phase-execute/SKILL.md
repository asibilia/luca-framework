...
After <%= branding.commandPrefix %>-learner returns, check the `process_data` flag passed by the lu orchestrator (fail-closed: absent flag = skip):
...
- Spawn `<%= branding.commandPrefix %>-executor` for each plan in wave (parallel Task calls, max 5 per batch)
- Wait for completion
...
#### 8.0.1. Collect Pre-Mortem Mitigations

If the phase discussion (Step 4) produced pre-mortem mitigations, collect them as additional review criteria to pass to ALL reviewers:
...
**Always spawn the 3 core reviewers** (dx-advocate, code-simplifier, code-architect). Conditionally spawn security-auditor if changed files match security patterns (see below). Architecture-lens and data-lens concerns are folded into the code-architect and dx-advocate base prompts respectively. Each reviewer resolves its model tier from the routing table based on complexity:
...
**Your focus:**
- Naming conventions, coding standards, Lodash vs native methods, snake_case API keys, import organization
- Schema-first parsing: ALL inputs must have Zod schema definitions. Defaults in schemas, NEVER in destructuring
- API snake_case: all API request/response payloads must use snake_case. Internal TypeScript uses camelCase
- Zod patterns: prefer safeParse() over parse(). Handle parse failures gracefully
- Type inference: types must be inferred from schemas using z.infer<typeof Schema>
- Data flow: verify transformations preserve fields, types, and state consistency
- Pre-mortem mitigations (if available): {PRE_MORTEM_MITIGATIONS}
...
**Your focus:**
- Architecture patterns, module boundaries, component structure, separation of concerns
- Dependency direction: imports must flow downward through tiers (T0 Foundation -> T1 Core -> T2 Entity -> T3 Build). Flag upward or cross-tier violations
- Entity isolation: agents, skills, rules domains must NEVER cross-import
- Barrel purity: every domain index.ts must contain ONLY re-export statements
- Structural invariants: no flat .ts files in domain root except index.ts
- Pre-mortem mitigations (if available): {PRE_MORTEM_MITIGATIONS}
...
# NOTE: Architecture-lens focus areas are folded into the code-architect prompt above.
# Data-lens focus areas are folded into the dx-advocate prompt above.
# This reduces the review team from 5-6 agents to 3-4 while preserving coverage.

```

**Do NOT proceed until ALL reviewer Tasks return.**

**Merge findings:** Combine all issues from reviewers, deduplicate by file:line.
...
**Reviewers:** dx-advocate, code-simplifier, code-architect{NEEDS_SECURITY ? ", security-auditor" : ""}
