...
- `<%= branding.commandPrefix %>-debugger` — Diagnoses root causes of UAT failures (parallel, one per issue)
- `<%= branding.commandPrefix %>-planner` — Creates fix plans from diagnosed gaps
- `<%= branding.commandPrefix %>-plan-checker` — Verifies fix plans before execution (max 3 iterations)
...
For each failed UAT test, spawn a <%= branding.commandPrefix %>-debugger agent:
...
After all debuggers return, spawn <%= branding.commandPrefix %>-planner in gaps mode:
...
Spawn <%= branding.commandPrefix %>-plan-checker to validate plans (iterate max 3 times):
