...
### Step 7.5: Spawn <%= branding.commandPrefix %>-learner

Gather all categorized concerns and verification results, then spawn <%= branding.commandPrefix %>-learner:
...
- **<%= branding.commandPrefix %>-learner failure:** Log warning, write empty learnings_captured array. This is an optional skill, so failure does not halt the chain.
