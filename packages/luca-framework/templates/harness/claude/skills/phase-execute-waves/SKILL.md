...
2. For each plan in the wave:
   - Spawn `<%= branding.commandPrefix %>-executor` sub-agent via Task() with the plan content and context
   - Wait for completion
...
- **<%= branding.commandPrefix %>-executor failure:** Log the failure, mark the plan as failed in the summary, and evaluate whether downstream plans can still execute.
...
- Spawn <%= branding.commandPrefix %>-executor for each plan — do NOT execute plans inline
