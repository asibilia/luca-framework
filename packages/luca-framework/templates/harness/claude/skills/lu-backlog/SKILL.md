# <%= branding.commandPrefix %>-backlog
...
<main>
# <%= branding.commandPrefix %>-backlog — Backlog Scan & Roadmap Revision
...
**If SWARM_ENABLED == false:** Use <%= branding.commandPrefix %>-pm-planner for WSJF scoring and phase placement.
...
2. **Spawn 3 specialists in parallel** via Task():
   - <%= branding.commandPrefix %>-roadmap-architect (architectural impact analysis)
   - <%= branding.commandPrefix %>-roadmap-prioritizer (WSJF scoring and prioritization)
   - <%= branding.commandPrefix %>-roadmap-qa (QA and testing gap analysis)
3. **Collect specialist ResultEnvelopes** (10-minute timeout, graceful degradation)
4. **Spawn synthesizer** (<%= branding.commandPrefix %>-roadmap-synthesizer) to merge analyses
...
After writing results, return to the lu orchestrator. The orchestrator will write `current_state: "scanned"` and invoke <%= branding.commandPrefix %>-phase-loop next.
</main>