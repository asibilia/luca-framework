# <%= branding.commandPrefix %>-phase-loop
...
<main>
# <%= branding.commandPrefix %>-phase-loop — Phase Execution, Milestone Gate, Summary
...
- `<%= branding.commandPrefix %>-planner` — Generate PLAN.md (parallel planning)
- `<%= branding.commandPrefix %>-executor` — Execute plans (parallel execution)
...
Re-read ROADMAP.md (may have been updated by <%= branding.commandPrefix %>-backlog):
...
After milestone-new creates the new roadmap, loop back to Step 1 (backlog scan) by re-invoking <%= branding.commandPrefix %>-backlog and then <%= branding.commandPrefix %>-phase-loop.
...
1. Running `<%= branding.commandSlash %>` again
