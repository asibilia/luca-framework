...
/**
 * Canonical map of every `luca` sub-command, keyed by CLI noun.
 *
 * Hoisted to module scope (and exported) so the stage-gate classifier's
 * registry-completeness test can bind cli.ts nouns to the classifier sets.
 * All values are lazy import thunks, so importing this module stays
 * side-effect-free.
 */
export const CLI_SUBCOMMANDS = {
    ...
    state: () =>
        import('./commands/write-surface/state').then((m) => m.stateCommand),
    phase: () =>
        import('./commands/write-surface/phase').then((m) => m.phaseCommand),
    plan: () =>
    ...
    brain: () =>
        import('./commands/write-surface/brain').then((m) => m.brainCommand),
    'pr-review': () =>
    ...
    budget: () =>
        import('./commands/write-surface/budget').then((m) => m.budgetCommand),
    confidence: () =>
