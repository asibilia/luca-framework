...
- [x] SEC-002: Add path containment guard in init.ts and build-deploy.ts (resolve(absPath).startsWith guard)
- [x] SEC-001: Validate hook command string paths with character allowlist regex in init.ts
- [x] SEC-003: Add explicit branding key allowlist in resolve-templates.ts
- [x] SEC-004: Use sanitizeJsonParse in build-deploy.ts loadBrandingContext
- [x] SEC-005: Replace Bun.spawnSync chmod with chmodSync in build-deploy.ts
...
- [x] DX-001: Rename `dirname` parameter to `segment` in template-transform.ts transformBrandingDirname
- [x] DX-002: Import defaultBranding from branding.ts in build-deploy.ts loadBrandingContext
- [x] DX-003: Replace node:path with pathe in resolve-templates.ts
- [x] DX-004: Add comment documenting shim indirection in build-deploy.ts imports
- [x] DX-005: Add try/catch around JSON.parse in build-compile.ts settings fragment
- [x] DX-006: Add @example to walkDir JSDoc in resolve-templates.ts
- [x] DX-008: Complete JSDoc for buildProposedHooksFromDeployed in init.ts
- [x] ARCH-001: Use barrel import (~/compilers) in build-compile.ts instead of direct \_\_helpers/
- [x] ARCH-002: Remove bun test references from qa-plan-generator.agent.ts
