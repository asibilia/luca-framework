[azukiazusa.dev](https://azukiazusa.dev/en/)

- [blog](https://azukiazusa.dev/en/blog)
- [about](https://azukiazusa.dev/en/about)
- [talks](https://azukiazusa.dev/en/talks)
- [recap](https://azukiazusa.dev/en/recap)

[ja](https://azukiazusa.dev/blog/claude-advisor-tool) [en](https://azukiazusa.dev/en/blog/claude-advisor-tool)

[RSS](https://azukiazusa.dev/en/rss.xml)[LLMS](https://azukiazusa.dev/en/llms.txt)[>](https://github.com/azukiazusa1/sapper-blog-app)
...
[Back to blog](https://azukiazusa.dev/en/blog)

This article was translated from Japanese by AI and may contain
inaccuracies. For the most accurate content, please refer to the [original Japanese version](https://azukiazusa.dev/blog/claude-advisor-tool).

![スモークサーモンのイラスト](https://images.ctfassets.net/in6v9lxmm5c8/45xu52pFThgYkbxW478soh/dde9493d0435fb29831338bbf2dfef4b/smoke-salmon_15757-768x591.png?q=50&fm=webp)

# Using Claude's Advisor Strategy to Optimize the Balance Between Performance and Cost

Apr 11, 2026

[#claude-code](https://azukiazusa.dev/en/tags/claude-code)
...
1. [Trying the Advisor Strategy in Claude Code](https://azukiazusa.dev/en/blog/claude-advisor-tool/#trying-the-advisor-strategy-in-claude-code)
2. [Summary](https://azukiazusa.dev/en/blog/claude-advisor-tool/#summary)
3. [References](https://azukiazusa.dev/en/blog/claude-advisor-tool/#references)
...
![](https://images.ctfassets.net/in6v9lxmm5c8/PA1imzWLCDBjx9ogS52ZM/50387efebb71bea8dd585dff57499a5e/image.png?q=50&fm=webp)
...
![](https://images.ctfassets.net/in6v9lxmm5c8/1X3q32rnRC8to3UqP1LKCL/c82d867017d3462ebc4b998c09b0ed83/%C3%A3__%C3%A3__%C3%A3_%C2%AA%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3__%C3%A3___2026-04-11_18.06.47.png?q=50&fm=webp)
...
### What problem is Claude's Advisor tool primarily designed to solve?

- Using only high-performance models at all times to finish every task as quickly as possible
...
The goal of the Advisor tool is not to always use a high-performance model, but to balance cost and capability by consulting one only when needed.

- Using a lightweight model for routine work while consulting a high-performance model only for difficult parts
...
This is the core value described in the article: let a lightweight model handle normal execution and bring in a stronger model only for complex problems.

- Running multiple high-performance models in parallel so lightweight models are unnecessary
...
The Advisor tool is not about parallelizing high-performance models. It is built around using lightweight models as the default.

- Discarding conversation history and starting a fresh session each time to reduce mistakes
...
The article explains that conversation history is used when consulting the advisor, so discarding history is not the point.


### According to the article, which statement about the \`advisor\` tool's arguments is correct?

- You must pass the model name as an argument to call the advisor
...
The article says you choose the model after running \`/advisor\`, not by passing the model name as a tool argument.

- You can pass a file path so only that file is shared with the advisor
...
The article does not describe any file path argument. It says the entire conversation history is shared.

- It takes no arguments, and the full conversation history is automatically transferred when you call it
...
The article explicitly states that the \`advisor\` tool itself takes no arguments and automatically transfers the full conversation history.

- You need to package the consultation content as JSON arguments
...
The article does not describe building JSON arguments. It says you simply call the tool.


## Contributors

[![azukiazusa1](https://avatars.githubusercontent.com/u/59350345?v=4)](https://github.com/azukiazusa1)

[> Suggest an edit on GitHub](https://github.com/azukiazusa1/sapper-blog-app/tree/main/contents/blogPost/claude-advisor-tool.md)

Share this article

[Share on X](https://x.com/share?url=https://azukiazusa.dev/en/blog/claude-advisor-tool&text=Using%20Claude%27s%20Advisor%20Strategy%20to%20Optimize%20the%20Balance%20Between%20Performance%20and%20Cost) [![Add to Hatena Bookmark](https://azukiazusa.dev/_app/immutable/assets/hatenabookmark_symbolmark.BhyWUNnR.png)](https://b.hatena.ne.jp/add?mode=confirm&url=https://azukiazusa.dev/en/blog/claude-advisor-tool&title=Using%20Claude%27s%20Advisor%20Strategy%20to%20Optimize%20the%20Balance%20Between%20Performance%20and%20Cost)

[Markdown version](https://azukiazusa.dev/en/blog/claude-advisor-tool.md)

## Related articles

[![具無しラーメンのイラスト](https://images.ctfassets.net/in6v9lxmm5c8/3e6uvd4NOdnb4DZMm4eEOG/1fe143e6637dc03d16845f4e29a7f3e4/ramen-without-toppings_22851-768x630.png?q=50&fm=webp)](https://azukiazusa.dev/en/blog/enable-claude-code-tool-search-to-reduce-mcp-token-usage)

Dec 30, 2025

## [Claude Code のツール検索ツールを有効にして MCP のトークン使用量を削減する](https://azukiazusa.dev/en/blog/enable-claude-code-tool-search-to-reduce-mcp-token-usage)

[![富山のブラックラーメンのイラスト](https://images.ctfassets.net/in6v9lxmm5c8/5b74BH8LnmSdnIrchExmLR/a27828355e02731923fb558045afc123/toyama-black-ramen_22871.png?q=50&fm=webp)](https://azukiazusa.dev/en/blog/try-react-grab-for-agents)

Dec 13, 2025

## [ブラウザから要素を選択してエージェントにコンテキストを提供する React Grab を試してみた](https://azukiazusa.dev/en/blog/try-react-grab-for-agents)

[![学生帽のイラスト](https://images.ctfassets.net/in6v9lxmm5c8/3HbASDkVChe9l3OtJh3j03/7a6aac529916499d9171140e55654c20/student-cap_17489-768x630.png?q=50&fm=webp)](https://azukiazusa.dev/en/blog/claude-code-learning-mode)

Aug 23, 2025

## [Claude Code の学習モードで自分の手でコードを書く練習をしよう](https://azukiazusa.dev/en/blog/claude-code-learning-mode)

© 2026 - Copyright azukiazusa, All Rights Reserved.

This site uses Google Analytics to analyze access. Google Analytics collects information about users' visits to the site, but does not collect personally identifiable information. For more details, please see [Google's use of information from sites and apps that use Google services – Policies and Terms](https://policies.google.com/technologies/partner-sites) .