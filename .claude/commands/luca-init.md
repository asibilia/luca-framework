...
This is distinct from the `luca init` CLI command (`npx luca init`), which wires up the per-project `.claude/` hooks and registers the MCP server. `/luca-init` runs *after* that, to capture the repo's working conventions into `.luca/config.json` (via `luca_preferences_write`) so the pipeline emits work that matches the project's existing style.
