...
Systematic debugging with persistent state across context resets. Use when user wants to debug an issue, mentions /lu-debug, or has a bug to investigate.
...
- Offer options:
  ...
  - "Plan fix" - suggest /lu-plan-phase --gaps
  - "Manual fix" - done
...
|-----------|--------|---------|
| Root cause found, fix needed | Plan the fix | `/lu-plan-phase {phase} --gaps` |
| Root cause found, quick fix | Fix directly | `/lu-quick` |
| Investigation inconclusive | Check project status | `/lu-progress` |
| Debug session saved | Resume later | `/lu-debug` (lists active sessions) |
