---
title: "Runtime X03: Backlog integration audit — v2 pipeline overlap resolution"
...
The runtime architecture (Phases A-E) and v2 pipeline improvements (v2-phase-1 through v2-phase-6) touch overlapping code. This todo resolves all sequencing conflicts with concrete decisions, documented in a single decision file.

## Task

### 1. Create decisions document

**File:** `docs/runtime-architecture/decisions/backlog-integration-decisions.md`

Write the following content (these are FINAL decisions, not discussion points):
...
- `docs/runtime-architecture/decisions/backlog-integration-decisions.md` exists with all 5 decisions
- Each decision has: question, decision, rationale, and impact
- No ambiguity remains about v2/runtime sequencing
- The `workflow.engine` config field is documented as the coexistence mechanism

## Notes

- This is a documentation-only task. No code changes.
- The decisions should be referenced by implementers of Phase A, v2-phase-6, and the config schema updates.
