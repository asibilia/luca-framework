---
title: "Runtime X08: Open questions resolution — concrete decisions for all unresolved design questions"
...
The cross-cutting concerns todo identified 5 open design questions that block or constrain implementation. Each needs a single recommended answer backed by research. This todo resolves all 5 with concrete decisions.

## Task

### 1. Create open questions resolution document

**File:** `docs/runtime-architecture/decisions/open-questions-resolved.md`
...
- `docs/runtime-architecture/decisions/open-questions-resolved.md` exists with all 5 questions resolved
- Each question has: options considered, decision, rationale, and implementation sketch
- No ambiguity remains — each decision is a single concrete answer
- Decisions are consistent with each other (e.g., Q4 migration strategy references Q1 DAG granularity)
- Decisions are consistent with other X-series todos (X03, X05, X06, X07)

## Notes

- These decisions should be treated as FINAL unless new information emerges that invalidates the rationale. The implementing agents should not re-debate these questions.
- If an implementing agent discovers that a decision is unworkable during implementation, they should document the specific failure and propose a revision — not silently change the approach.
