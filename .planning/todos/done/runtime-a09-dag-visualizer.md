---
title: "Runtime A09: DAG-to-topology transformer for luca-observer"
area: workflow
created: 2026-03-24
...
Instead of generating static Mermaid diagrams, the DAG visualizer transforms a `WorkflowDAG` into the `WorkflowTopologyResponse` format that luca-observer's React Flow workflow editor already consumes. This lets us reuse the existing interactive graph (stage-group containers, agent/skill/gate nodes, auto-layout, complexity filtering, minimap, sidebar) with zero new UI work.

The existing workflow editor in luca-observer (`/workflow-editor`) uses:

- `@xyflow/react` v12 with custom node types (stage-group, agent, skill, gate)
- Topology data via `WorkflowTopologyResponseSchema` (Zod, snake_case)
- Auto-layout via `applyGroupedColumnLayout()`
- Static topology from `packages/luca-observer/lib/workflow-topology.ts`

This task creates a transformer that converts typed `WorkflowDAG` definitions into the same topology format, so the observer can visualize DAG definitions alongside (or instead of) the static topology.
...
Replace the `// Added by A09` placeholder under DAG Visualizer with:
...
- [ ] `dagToTopology()` returns an object with `nodes`, `edges`, `stages` arrays
- [ ] Each unique category in the DAG produces one `stage-group` container node
- [ ] Each DAG step produces a child node with `parent_id` pointing to its stage group
- [ ] Child node positions use the same grid layout constants as `workflow-topology.ts`
- [ ] Stage-group containers are stacked vertically with `GROUP_Y_GAP` spacing
- [ ] Steps with `gate` category produce nodes with `node_type: "gate"`
- [ ] Steps with handlers in `KNOWN_SKILL_HANDLERS` produce `node_type: "skill"`
- [ ] All other steps produce `node_type: "agent"`
- [ ] `dependsOn` relationships produce edges with `edge_type: "data-flow"`
- [ ] Spine edges connect consecutive stage-group containers
- [ ] Output is structurally compatible with `WorkflowTopologyResponseSchema` (validated by luca-observer at the API boundary)
- [ ] Barrel index only contains re-export statements

## Notes

- Depends on: A01 (directory structure), A02 (WorkflowDAG, WorkflowStep, StepCategory types), A03 (step contracts for handler names)
- No external dependencies — pure data transformation
- The types mirror `packages/luca-observer/lib/workflow-types.ts` but are defined locally to avoid cross-package imports (Next.js build boundary). Structural compatibility is validated by Zod at the observer's API boundary.
- The container sizing constants (`HEADER_HEIGHT`, `COLUMN_WIDTH`, etc.) are duplicated from `workflow-topology.ts` — these are stable layout values unlikely to change
- The `KNOWN_SKILL_HANDLERS` set should be updated when new skills are added to the DAG
- The old Mermaid approach is replaced entirely — if Mermaid output is needed later, it can be generated from the topology format
