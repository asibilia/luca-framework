---
title: "Runtime C01: Eval domain scaffolding + schemas"
area: eval
created: 2026-03-24
...
Create the `src/eval/` domain structure following Archetype B (Core Domain) conventions. This domain sits at T1 Core tier, meaning it can import from T0 (shared, complexity) and T1 (other core domains like iteration, harness) but NOT from T2 (agents, skills, rules) or T3 (compilers, hooks).

This task creates only the directory structure, Zod schemas, and an empty barrel. No logic.

## Files to Create

### 1. `src/eval/__schemas/eval.schemas.ts`

All schemas use snake_case field names per API conventions. Types are inferred via `z.infer<typeof Schema>`.
...
Pure barrel file. Only re-export statements.
...
**Note:** The barrel will be extended in C02-C05 as helper modules are added.

## Directories to Create

```
mkdir -p src/eval/__schemas
mkdir -p src/eval/__helpers
mkdir -p src/eval/suites
```
...
# Barrel is pure re-exports (no logic)
...
# Domain boundary check (requires adding eval to tier map first - see C10)
# Deferred to C10
```

## Notes

- This task does NOT create helper files or eval suites -- those are C02-C09.
- The `custom` code grader strategy stores its scoring function at runtime, not in the schema (functions are not serializable). The runner (C03) will accept a `Map<string, CustomGraderFn>` for custom graders.
- `judge_model` default is `"claude-haiku-4-5-20250514"` per the research cost analysis recommending Haiku 4.5.
