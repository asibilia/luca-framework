---
title: "Runtime A06: Implement DAG validator with 5 static checks"
area: workflow
created: 2026-03-24
source: docs/runtime-architecture/dag-workflow-engine.md
depends_on: [A01, A02, A05]
phase: runtime-a
estimated_files: 2
---
...
Implement static analysis of DAG definitions at build time. The validator performs 5 checks: no cycles, all dependsOn references exist, schema compatibility between connected steps, no orphaned steps, and parallel group validation. It delegates cycle detection to the topological sorter (A05) and returns a `ValidationResult` with errors and warnings.

## Task

### Files to Create

#### `src/workflow/__helpers/dag-validator.ts`
...
Replace the `// Added by A06` placeholder comment under `DAG Validator` with actual exports:
...
- [ ] Cycle detection: DAG with A -> B -> A returns `valid: false` with error type `"cycle"`
- [ ] Missing dependency: step depending on nonexistent ID returns error type `"missing-dependency"`
- [ ] Schema mismatch: step B expects field not in step A's output produces warning type `"schema-mismatch"`
- [ ] Orphaned step: step unreachable from roots produces warning type `"orphaned-step"`
- [ ] Invalid parallel group: group referencing nonexistent step produces error type `"invalid-parallel-group"`
- [ ] Valid DAG: linear pipeline returns `valid: true` with empty errors and warnings
- [ ] Barrel index only contains re-export statements

## Notes

- Depends on: A01 (directory structure), A02 (schemas/types), A05 (topologicalSort for cycle detection)
- Schema compatibility (Check 3) produces warnings, not errors, because it is a best-effort static check. Runtime validation at step boundaries is the authoritative gate.
- Orphaned steps (Check 4) also produce warnings, not errors — a disconnected step is unusual but may be intentional (e.g., a step guarded by a condition that makes it unreachable in some configurations).
- The `getZodObjectShape` helper uses duck-typing to check if a value is a ZodObject with a `.shape` property, rather than importing `z.ZodObject` and using `instanceof`. This avoids issues with different Zod versions or bundling.
