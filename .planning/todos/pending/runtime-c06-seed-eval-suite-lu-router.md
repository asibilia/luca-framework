---
title: "Runtime C06: Seed eval suite for lu-router"
area: eval
created: 2026-03-24
...
Create 25 eval cases for lu-router complexity classification. lu-router receives a task description (plus cognitive report context) and outputs a complexity level: `TRIVIAL | SIMPLE | MODERATE | COMPLEX | CRITICAL`.

The grading strategy is `code` with `set_membership` or `exact_match`. For ambiguous cases where two adjacent levels are both acceptable, use `set_membership` with both levels.

**lu-router input contract** (from `src/agents/general/lu-router.agent.ts`):

- Receives: task description string, cognitive report with memory recall, intuition flags
- Outputs: structured routing decision with `Complexity: {LEVEL}` classification

For eval purposes, the input is a record with:

- `task_description`: string (the task to classify)
- `cognitive_report`: string (optional context, can be empty for deterministic cases)

The expected output is the complexity level string.

Since lu-router is a prompt-based agent (not a function), the eval runner would need to call the LLM with the compiled agent prompt + input. For Phase C, we test the classification LOGIC by providing the input and expected output. The actual LLM call integration happens when Phase B's API adapter is available. For now, these cases serve as a labeled dataset.

## Files to Create

### 1. `src/eval/suites/lu-router.eval.ts`
...
**Edge cases with `set_membership` grading** (3 of the 25 cases above should use this pattern). Replace the following 3 cases' `code_grader_config` to accept adjacent levels:

- `router-simple-002` ("Add a new Zod schema field"): Could be TRIVIAL or SIMPLE. Change to:
...
- `router-moderate-003` ("Refactor a function and update 3-4 consumers"): Could be SIMPLE or MODERATE. Change to:
...
- `router-complex-004` ("Cross-cutting concern"): Could be COMPLEX or CRITICAL. Change to:
  ```typescript
  code_grader_config: {
    strategy: "set_membership",
...
- These cases are a **labeled dataset**, not runtime tests. They define the ground truth for lu-router classification.
- The `cognitive_report` field simulates the context lu-router receives from lu-cognition. Empty string means no cognitive context (e.g., for trivial tasks where pre-flight is lite).
- When the Phase B API adapter is available, these cases will be used with real LLM calls. Until then, they serve as the eval case definition.
- The `output_path: "complexity"` tells the code grader to extract the `complexity` field from the agent output object before comparing.
