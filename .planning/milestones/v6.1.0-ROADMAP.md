...
- [x] #1: Fix compile.ts barrel bypass — use ~/adapters/claude sub-barrel
- [x] #2: Fix dag-serializer.ts — replace require('node:fs') with top-level import, remove dead Bun.file() code
- [x] #4: Fix composite-grader.ts — pass caseId to customFns lookup
- [x] #5-7: Fix adapter deep \_\_schemas/ imports — export BaseAgent/BaseSkill/BaseRule from entity barrels, use barrel imports in adapters
...
- [x] #3: Extract shared buildSuccessorsMap from dag-validator/dag-sorter
- [x] #8: Remove eval suite re-exports from eval barrel
- [x] #9: Extract makeFailResult factory for eval graders
- [x] #10: Collapse createMockAdapter into createMockAdapterWithResponses
- [x] #13: Rename duplicate TokenUsageSchema (AdapterTokenUsage vs EvalTokenUsage)
- [x] #14-15: Move LlmAdapter and RunEvalOptions from **helpers/ to **schemas/
...
- [x] #11: Extract shared convergence state machine for review skills
- [x] #12: Extract shared SDK type guard predicate in api-executor
- [x] #16: Remove duplicate section header in workflow/index.ts
- [x] #17: Rename workflow Adapter to WorkflowAdapter
- [x] #18: Extract emitRuleMarkdown to rule-emitter.ts
- [x] #19: Remove redundant detectAdapter second loop
- [x] #20: Extract shared loadReportFile helper in eval-reporter
- [x] #21: Add side-effect import comment to adapters barrel
- [x] #22: Fix lu-verifier.eval.ts to use barrel import
- [x] #23: Remove .ts extensions from workflow imports
- [x] #24: Remove unreachable event types from CANONICAL_EVENTS
