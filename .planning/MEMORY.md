...
| [Phase 41] Bridge path rewire from src/ to packages/                  | Package extraction migration | [decisions, architecture]               | All references to `src/state-machine/bridge.ts` and `src/state-machine/cli.ts` rewired to `packages/luca-state/src/bridge.ts` and `packages/luca-state/src/cli.ts` across skills, agents, rules, hooks, and templates. Old `src/state-machine/` directory preserved with deprecation README for backward compatibility. Test files in `src/state-machine/__tests__/` also rewired. Verified: zero old-path references in active code, 347 package tests + 2107 project tests pass | 2026-02-16 |
