...
- **[Phase 23] Comprehensive E2E summary test as final gate**: A single "load readiness" test that aggregates ALL validation checks (manifest, structure, frontmatter, hooks, marketplace consistency) into one pass/fail with structured issue reporting provides a definitive answer. Individual tests catch specific issues in isolation; the summary test catches integration-level failures across components. The structured issues array enables detailed debugging when the gate fails
  ...
  - **Confidence**: High
  - **Added**: 2026-02-12
- **[v1.3.1] Scope-first skill naming for plugin namespace clarity**: When skills are distributed under a plugin namespace (`luca:`), drop the framework prefix (`lu-`) from skill names and adopt scope-first naming (e.g., `lu-execute-phase` → `phase-execute`). This produces clear plugin commands (`/luca:phase-execute`) without redundant `lu` in both the namespace and the name. Group related skills by scope prefix (phase-, milestone-, session-, config-, git-, todo-) for natural tab-completion ordering
  - **When to use**: When renaming skills for plugin distribution or any namespace-prefixed system
  - **Tags**: [patterns, conventions, architecture]
  - **Confidence**: High
  - **Added**: 2026-02-12
- **[v1.3.1] Command stub files for plugin autocomplete**: Plugin skills only appear in "/" autocomplete if a corresponding `commands/<name>.md` file exists with YAML frontmatter containing a `description` field and a non-empty body. Empty-body command files cause API 400 errors. The body content can be minimal (e.g., "Invoke the X skill to execute this command."). Exclude internal/reference skills (`rule-*`, `workflow-start`) from commands
  - **When to use**: When generating plugin command files for Claude Code autocomplete discovery
  - **Tags**: [patterns, architecture, conventions]
  - **Confidence**: High
  - **Added**: 2026-02-12
...
- **[Phase 27] Surgical schema validation over blanket size limits**: Instead of adding generic max-length constraints to all schema fields, audit actual field usage to determine realistic bounds. Phase 27: description field set to .max(500) (used by CLI help), keywords set to .max(20) items with .min(1).max(50) per item (used by marketplace search). Bounds justified by use case, not arbitrary. Prevents over-restrictive validation. Applied to pluginManifestSchema (SEC-04). Validated in Phase 27 (SEC-04 requirement, 1 commit)
  ...
  - **Confidence**: High
  - **Added**: 2026-02-13
- **[Phase 43] Profile-based rule organization with config-driven loading**: Organize code style rules under named tech stack profiles (`src/rules/profiles/<stack>/`) with config toggles (`workflow.opinionated_guidelines`, `workflow.tech_stack_profiles`) and lazy factory loading. The existing build pipeline (registries, compilers, drift detection) integrates cleanly when profiles export rules through the same registry interface. Validated in Phase 43 (19 rules default, 11 with opinionated off, 2091 tests pass)
  - **When to use**: When adding convention/rule organization that needs to scale across multiple tech stacks
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, lu-codebase-mapper, build pipeline]
  - **Tags**: [patterns, architecture, conventions]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 43] Lazy factory pattern for conditional rule loading**: Profile rules use lazy factories (`() => new RuleClass()`) instead of eagerly instantiated rule objects. This avoids loading rule definitions for inactive profiles and defers construction cost until the profile is actually selected by config. Combined with config toggle checks at load time, the rule count scales only with active profiles. Validated in Phase 43 (TypeScript profile: 8 rules via lazy factories)
  - **When to use**: When registry entries should only be constructed when their containing group/profile is active
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, build pipeline, rule system]
  - **Tags**: [patterns, architecture, performance]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 43] git mv for rule file migration preserves history**: When reorganizing files between directories (e.g., `src/rules/general/` to `src/rules/profiles/typescript/`), use `git mv` instead of manual move+add. This preserves file history in git log and blame, which is critical for understanding why a rule was written. Validated in Phase 43 (8 TS rules migrated with history preserved)
  - **When to use**: When reorganizing existing files into new directory structures
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, general]
  - **Tags**: [patterns, conventions, coding]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 40] Zero-dependency standalone package via inlined utilities**: When extracting a package with framework dependencies (complexity-utils, cli-utils, budget-utils), inline self-contained copies rather than cross-package imports. The resulting package has zero framework imports and can be versioned/distributed independently. Validated in Phase 40 (luca-state package: 347 tests pass, 88.80% coverage, zero framework imports)
  - **When to use**: When extracting a module into a standalone package that should have no external service dependencies
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, lu-package-extractor]
  - **Tags**: [patterns, architecture, packaging]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 40] Dual-source approach for package extraction prevents regression**: When a module exists in both framework (`src/`) and extracted package (`packages/luca-state/`), keep both copies. The framework retains the original as a fallback; the package has its own inlined utilities. This dual-source model enables gradual migration without breaking the framework. If a breaking change occurs, rollback is trivial (delete package, keep framework working). Validated in Phase 40 (bridge.ts → cli.ts works with both framework and package sources)
  - **When to use**: When extracting a package from a monorepo and need backward compatibility
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, lu-package-extractor]
  - **Tags**: [patterns, architecture, migration]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 40] Import rewiring audit for package extraction**: Four files need import rewiring during extraction: persistence.ts (Result type), snapshot.ts (escapeRegex), bridge.ts (getArg/hasFlag), cli.ts (getArg/hasFlag). Other modules (machine.ts, actions.ts, events.ts, actors/) have intra-module imports only and require zero rewiring. Audit every import before extraction to identify: (1) cross-package imports (rewrite), (2) intra-module imports (unchanged), (3) external packages (keep as-is). Validated in Phase 40 (4 files rewired, 347 tests pass)
  - **When to use**: When extracting a module into a standalone package
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, lu-package-extractor]
  - **Tags**: [patterns, architecture, verification]
  - **Confidence**: High
...
| [Phase 43] Synchronous readFileSync for config at module init         | Profile config loading       | [decisions, architecture, stack]        | `loadProfileConfig()` uses `require("fs").readFileSync` instead of async `Bun.file().text()` because it runs at module initialization time during rule registry construction. Async would require top-level await or restructuring the entire registry initialization chain. Tradeoff: blocking I/O at startup for a single small JSON file (~1KB) is acceptable for build-time tooling. Not appropriate for runtime server code                                                  | 2026-02-16 |
| [Phase 43] Empty placeholder profiles for future stacks               | Multi-stack extensibility    | [decisions, architecture, conventions]  | Created empty profile directories and registries for python, go, rust with zero rules. This establishes the directory structure and registry pattern so future contributors can add rules incrementally without needing to understand the profile infrastructure. The build pipeline handles empty profiles gracefully (no rules loaded, no errors)                                                                                                                               | 2026-02-16 |
| [Phase 40] Extract package as self-contained module, not thin wrapper | Package extraction strategy  | [decisions, architecture]               | luca-state package includes inlined copies of cli-utils, complexity-utils, budget-utils. Creates zero cross-package imports and enables independent versioning. Tradeoff: utility changes in framework don't auto-propagate to package (must manually sync). Chosen because the utilities are stable and self-contained. Alternative (framework as runtime dependency) would complicate distribution and create circular dependencies                                             | 2026-02-16 |
| [Phase 40] Keep framework copy during package extraction              | Backward compatibility       | [decisions, architecture]               | After extracting luca-state package, the original `src/state-machine/` remains in the framework. This enables gradual migration where framework imports can use local copy OR remote package. Provides rollback path if package extraction has issues. Over time, framework should deprecate internal copy, but this dual-source approach prevents all-or-nothing cutover risk                                                                                                    | 2026-02-16 |
| [Phase 41] Bridge path rewire from src/ to packages/                  | Package extraction migration | [decisions, architecture]               | All references to `src/state-machine/bridge.ts` and `src/state-machine/cli.ts` rewired to `packages/luca-framework/src/state/bridge.ts` and `packages/luca-framework/src/state/cli.ts` across skills, agents, rules, hooks, and templates. Old `src/state-machine/` directory preserved with deprecation README for backward compatibility. Test files in `src/state-machine/__tests__/` also rewired. Verified: zero old-path references in active code, 347 package tests + 2107 project tests pass | 2026-02-16 |
...
- **[Phase 27] Recursive deletion functions need root path guards**: cleanDirectory() in build-utils.ts uses Bun.file().delete() recursively. Without a check that the target path is within expected bounds (e.g., always inside `./.claude/`, `./.cursor/`, or `./dist/`), a misconfiguration or parameter injection could delete /home, /var, or other critical system directories. Phase 27 added assertSafeCleanTarget() guard that validates the absolute path starts with one of the expected output roots. Caught by harness tests (SEC-03 requirement). Pattern: guard all recursive deletion with path prefix validation
  ...
  - **Confidence**: High
  - **Added**: 2026-02-13
- **[Phase 43] Rule registry completeness tests hardcode directory paths**: The existing rule registry test in `__tests__/src/rules/rule-registry.test.ts` only scanned `src/rules/general/` for rule files. When rules were migrated to `src/rules/profiles/typescript/`, the test continued passing but with an incorrect count because it never scanned the profiles directory tree. Any test that validates registry completeness by scanning filesystem directories must be updated when the directory structure changes. Pattern: use glob patterns that cover the full rule tree (`src/rules/**/*.rule.ts`) rather than hardcoded single-directory scans
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, lu-verifier, testing]
  - **Tags**: [pitfalls, testing, architecture]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 43] Record<string, T> indexing returns T | undefined in strict mode**: TypeScript's strict null checks mean `Record<string, TechStackProfile>` indexed by a string key returns `TechStackProfile | undefined`, not `TechStackProfile`. Test code accessing `profileRegistry["typescript"]` needs non-null assertion (`!`) or an explicit undefined check. This is correct TypeScript behavior but catches developers off guard when the registry is known to contain the key. Solution: use non-null assertion in tests where the key is validated by prior assertions, or use a type-narrowing helper
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, testing]
  - **Tags**: [pitfalls, coding, testing]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 40] Package extraction requires barrel export registration**: When extracting a module into a standalone package, ensure the main `index.ts` (barrel export) includes ALL exports that external consumers need. Phase 40: first Wave 1 draft missed `persistence.ts` from the barrel. Consumers of the package (e.g., CLI bridge) couldn't import it. Always audit the public API: (1) list all modules the package exposes, (2) verify each appears in barrel, (3) test external imports before finalizing
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, lu-package-extractor]
  - **Tags**: [pitfalls, architecture, packaging]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 40] Test coverage gaps for inlined utility functions**: When inlining utilities (cli-utils.ts: escapeRegex, getArg, hasFlag) into a package, coverage for those functions depends on how heavily the package code exercises them. cli-utils coverage at 33% (only escapeRegex heavily used by snapshot.ts; getArg/hasFlag only tested at package CLI level). Budget-utils at 71% (assessBudget() edge cases not covered by existing tests). Manual gap audit required: identify what edge cases the package doesn't exercise, then add targeted tests. Don't rely on line coverage % alone — coverage % can be misleading when inlined utilities have dormant code paths
  - **Agent**: lu-executor
  - **Relevant to**: [lu-executor, testing, lu-package-extractor]
  - **Tags**: [pitfalls, testing, verification]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 42] Dual-copy state machine maintenance until Phase 41 integration**: The codebase has two copies of state machine code: `packages/luca-framework/src/state/` (extracted package) and `src/state-machine/` (framework). Both contain types.ts, machine.ts with identical logic. Both copies must be kept in sync during development. During types/machine changes, update both locations. Phase 41 will complete the integration rewire to use only the package copy. Workaround: after changes to either location, run `bun run build:all` and full test suite to catch inconsistencies. Temporary state until Phase 41 resolves this design debt. Validated in Phase 42 (memory bridge updates confirmed working with both source locations)
  - **Agent**: lu-executor
...
- **[Phase 42] Suspended vs paused state semantics**: Suspended state is triggered by context exhaustion or intentional pause (high context usage, workflow hold). Paused state is triggered by human verification hold (waiting for approval). Different semantics, different resume flows. Suspended resumes with fresh context and prior checkpoint. Paused resumes from exact checkpoint with same context. The state machine differentiates these for appropriate recovery behavior. Validated in Phase 42 (both states implemented with distinct resume paths)
  ...
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 41] Plan-checker catches real gaps even at moderate complexity**: Plan-checker identified 2 critical gaps in Phase 41 execution: missing `cli.test.ts` from test rewire, and `.cursor/luca/workflows/cognitive-preflight.md` not regenerated by build. These were legitimate oversights that manual review missed. Pattern: always run plan-checker before execution, even at MODERATE complexity where it's technically optional per complexity gating. The tool's specificity provides value disproportionate to its cost
  - **When to use**: When transitioning from planning to execution phase, especially for rewire-heavy or bulk-modification tasks
  - **Tags**: [patterns, verification, planning]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 41] Large-scale string substitution rewires benefit from grep sweep verification**: Phase 41 rewire of `src/state-machine/bridge.ts` → `packages/luca-framework/src/state/bridge.ts` (83 source files) was validated by running a final grep sweep: `grep -r "src/state-machine/bridge\|src/state-machine/cli" src/ --include="*.ts" --include="*.skill.ts"`. Returned zero results, confirming no old references remained. Pattern: for bulk find-replace operations affecting core infrastructure, follow with exhaustive grep sweep of affected directories to catch missed/nested references
  - **When to use**: After completing large-scale file path or import rewires that touch 50+ files
  - **Tags**: [patterns, verification, refactoring]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 41] Plan file naming convention mismatch**: lu-planner creates plan files as `PLAN-01.md` and `PLAN-02.md`, but lu-execute-phase expects `*-PLAN.md` pattern (e.g., `41-01-PLAN.md`). The mismatch forced manual file renaming in Phase 41 before execution could proceed. Mitigation: use grep to verify expected plan file names before starting phase execution, or update lu-planner to match the lu-execute-phase naming convention. Pattern: naming conventions that cross agent boundaries should be validated during plan verification
  - **Agent**: lu-planner, lu-execute-phase
  - **Relevant to**: [lu-planner, lu-execute-phase, lu-phase-researcher]
  - **Tags**: [pitfalls, planning, conventions]
  - **Confidence**: High
  - **Added**: 2026-02-16
- **[Phase 41] .cursor/luca/ deployed files not regenerated by build:all**: Files in `.cursor/luca/workflows/` (e.g., `cognitive-preflight.md`) are deployed during project initialization and are NOT included in the `bun run build:all` regeneration pipeline. Changes to these files require manual updates. The drift detection system does not monitor them because they have no corresponding source in `src/`. Mitigation: either (1) add source files to `src/workflows/` and update the build pipeline to generate `.cursor/luca/workflows/`, or (2) document `.cursor/luca/workflows/` as manually-maintained and update plan checklists to include them. Phase 41 discovered this when plan-checker flagged the file as manually modified but not regenerated
  - **Agent**: lu-executor, build pipeline
  - **Relevant to**: [lu-executor, lu-plan-checker, build system]
  - **Tags**: [pitfalls, architecture, conventions]
  - **Confidence**: High
  - **Added**: 2026-02-16
